...
function relCwdFor(absCwd) {
    if (absCwd === current.root) return './';
    if (path.isAbsolute(absCwd) && !absCwd.startsWith(current.root)) return `${absCwd}/`;
    return `${path.relative(current.root, absCwd)}/`;
}
...
function runHeadless({ entries, context, model, instruction }) {
    ...
    const { status, stdout: out } = ide.spawnHeadless(prompt, model, { cwd: current.root });
    return exit(status, () => {
        if (status !== 0) {
