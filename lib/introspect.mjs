...
async function traceBinding(graph, qualifiedName) {
    const t = new Trace(graph, qualifiedName);
    return t.forward();
}

class Trace {
    constructor(graph, qualifiedName) {
        this.graph = graph;
        this.modules = graph.modules;
        this.bindings = graph.bindings;
        this.byExport = graph.byExport;
        ...
        this.namePath = parts.slice(1);
        this.targetName = this.namePath.length > 0 ? this.namePath[0] : qualifiedName;
        this.targetModule = null;

        if (qualifiedName.includes('::')) {
            this.targetModule = resolveModulePathInGraph(parts[0], this.modules);
        }
    }

    resolveTarget() {
        if (this.targetModule) return this.targetModule;
        const producers = this.byExport.get(this.targetName) || [];
        if (producers.length > 0) {
            this.targetModule = producers[0].module;
            return this.targetModule;
        }
        for (const [relPath, file] of this.files) {
            const decl = file.declarations.find((d) => d.name === this.targetName);
            if (decl) {
                this.targetModule = relPath;
                return this.targetModule;
            }
        }
        return null;
    ...
    async forward() {
        if (!this.targetModule) {
            this.targetModule = this.resolveTarget();
        }
        if (!this.targetModule) {
            this.issues.push({
                kind: 'unresolved-name',
                message: `"${this.targetName}" not found in any module`
            });
            return { chain: this.chain, issues: this.issues };
        }

        if (this.namePath.length <= 1) {
            await this._walk(this.targetModule, this.targetName, 0);
            return { chain: this.chain, issues: this.issues };
        }

        const file = this.files.get(this.targetModule);
        if (!file) {
            this.issues.push({
                kind: 'missing-module',
                file: this.targetModule,
                message: `module "${this.targetModule}" not in graph`
            });
            return { chain: this.chain, issues: this.issues };
        }

        const outerDecl = file.declarations.find((d) => d.name === this.namePath[0]);
        if (!outerDecl) {
            this.issues.push({
                kind: 'unresolved-name',
                file: this.targetModule,
                message: `"${this.namePath[0]}" not found as a declaration in "${this.targetModule}"`
            });
            return { chain: this.chain, issues: this.issues };
        }

        this.visited.add(`${this.targetModule}::${this.namePath[0]}`);
        this.chain.push({
            file: this.targetModule,
            name: this.namePath[0],
            kind: 'declaration',
        ...
        await this._walkLocal(this.targetModule, outerDecl, this.namePath.slice(1), 1, content);
        return { chain: this.chain, issues: this.issues };
    }

    async _walk(modPath, name, depth) {
        ...
        const b = this.bindings.get(modPath);
        const file = this.files.get(modPath);
        ...
        if (b && b.exports && b.exports[name]) {
            ...
            this.chain.push({
                file: modPath, name, kind: 'export',
                line: exportInfo.line, type: exportInfo.type, depth
            });
            ...
            if (exportInfo.type === 'reexport' && exportInfo.source) {
                if (mod) {
                    ...
                    if (targetAbs) {
                        const targetRel = rarebert.relPath(targetAbs);
                        await this._walk(targetRel, name, depth + 1);
                    } else {
                        this.issues.push({
                            kind: 'broken-import',
                            file: modPath,
        ...
        if (file) {
            ...
            if (decl) {
                this.chain.push({
                    file: modPath, name, kind: 'declaration',
                    line: decl.startLine, declKind: decl.kind, depth
                });

                const step = await resolveOneStep(decl, file, this.graph);

                for (const imp of step.fromImports) {
                    this.chain.push({
                        file: modPath, name: imp.localName, kind: 'import',
                        line: imp.line, source: imp.source,
                        resolvedPath: imp.resolvedPath, depth: depth + 1
                    });
                    if (imp.resolvedPath) {
                        await this._walk(imp.resolvedPath, imp.binding || imp.localName, depth + 2);
                    } else if (imp.source && (imp.source.includes('/') || imp.source.startsWith('.'))) {
                        this.issues.push({
                            kind: 'broken-import', file: modPath, line: imp.line,
                ...
                for (const siblingDecl of step.fromDecls) {
                    this.chain.push({
                        file: modPath, name: siblingDecl.name, kind: 'declaration',
                        line: siblingDecl.startLine, declKind: siblingDecl.kind, depth: depth + 1
                    });
                    const siblingStep = await resolveOneStep(siblingDecl, file, this.graph);
                    for (const imp of siblingStep.fromImports) {
                        this.chain.push({
                            file: modPath, name: imp.localName, kind: 'import',
                            line: imp.line, source: imp.source,
                            resolvedPath: imp.resolvedPath, depth: depth + 2
                        });
                        if (imp.resolvedPath) {
                            await this._walk(imp.resolvedPath, imp.binding || imp.localName, depth + 3);
                        }
                    }
                }
                ...
                for (const unresolved of step.unresolved) {
                    this.issues.push({
                        kind: 'unresolved-name', file: modPath, line: decl.startLine,
            ...
            if (b && b.exports && b.exports[name] && b.exports[name].type !== 'reexport') {
                ...
                if (content) {
                    ...
                    if (declMatch) {
                        ...
                        this.chain.push({
                            file: modPath, name, kind: 'declaration',
                            line: lineNum, declKind: declMatch[0].match(/class/) ? 'class' :
                                   declMatch[0].match(/function/) ? 'function' : 'const', depth
                        });
                        return;
                    }
                }
                this.chain.push({
                    file: modPath, name, kind: 'local-definition',
                    line: b.exports[name].line, depth
                });
                return;
            }
        }
        ...
        if (b && b.imports) {
            ...
            if (imp) {
                this.chain.push({
                    file: modPath, name, kind: 'import',
                    line: imp.line, source: imp.source, depth
                });
                if (mod) {
                    const targetAbs = resolveImportPath(mod.abs, imp.source);
                    if (targetAbs) {
                        const targetRel = rarebert.relPath(targetAbs);
                        await this._walk(targetRel, imp.binding || name, depth + 1);
                    } else if (imp.source && (imp.source.includes('/') || imp.source.startsWith('.'))) {
                        this.issues.push({
                            kind: 'broken-import', file: modPath, line: imp.line,
        ...
        this.issues.push({
            kind: 'unresolved-name', file: modPath,
            message: `"${name}" not found in "${modPath}"`
        });
    }

    async _walkLocal(modPath, parentDecl, innerNames, depth, content) {
        const file = this.files.get(modPath);
        ...
        if (!lang || typeof lang.extractLocalMembers !== 'function') {
            this.issues.push({
                kind: 'no-local-trace', file: modPath,
        ...
        if (!local) {
            this.issues.push({
                kind: 'unresolved-name', file: modPath, line: parentDecl.startLine,
        ...
        this.chain.push({
            file: modPath, name, kind: 'local-declaration',
            declKind: local.kind, line: local.startLine, depth
        });

        if (!isLast) {
            await this._walkLocal(modPath, local, innerNames.slice(1), depth + 1, content);
            return;
        }
        ...
        for (const refName of referenced) {
            ...
            if (sibling) {
                this.chain.push({
                    file: modPath, name: refName, kind: 'local-declaration',
                    declKind: sibling.kind, line: sibling.startLine, depth: depth + 1
                });
                continue;
            }
            ...
            if (imp) {
                this.chain.push({
                    file: modPath, name: refName, kind: 'import',
                    line: imp.line, source: imp.source,
                    resolvedPath: imp.resolvedPath, depth: depth + 1
                });
                if (imp.resolvedPath) {
                    const walkKey = `${imp.resolvedPath}::${imp.binding || imp.localName}`;
                    if (!this.visited.has(walkKey)) {
                        this.visited.add(walkKey);
                        await this._walk(imp.resolvedPath, imp.binding || imp.localName, depth + 2);
                    }
                }
                continue;
            ...
            if (topDecl) {
                this.chain.push({
                    file: modPath, name: refName, kind: 'declaration',
                    line: topDecl.startLine, declKind: topDecl.kind, depth: depth + 1
                });
                const walkKey = `${modPath}::${refName}`;
                if (!this.visited.has(walkKey)) {
                    this.visited.add(walkKey);
                    await this._walk(modPath, refName, depth + 2);
                }
                continue;
            }
            this.issues.push({
                kind: 'unresolved-name', file: modPath, line: local.startLine,
    ...
    usage() {
        ...
        const resolveProducer = (relPath, info) => {
            const key = `${relPath}:${name}`;
            if (visited.has(key)) return;
            visited.add(key);
            if (info.type === 'reexport' && info.source) {
                const mod = this.modules.get(relPath);
                const targetRel = this._resolveImportSource(mod, info.source);
                if (!targetRel) {
                ...
                const target = this.bindings.get(targetRel);
                if (target && target.exports[name]) {
                    resolveProducer(targetRel, target.exports[name]);
                } else {
        ...
        const decls = this.byExport.get(name) || [];
        for (const decl of decls) {
            const b = this.bindings.get(decl.module);
            if (!b) continue;
            resolveProducer(decl.module, b.exports[name]);
        }

        const producerSet = new Set(producers.map((p) => p.module));
        const reachableCache = new Set();
        const isReachable = (relPath) => {
            if (producerSet.has(relPath)) return true;
            if (reachableCache.has(relPath)) return false;
            reachableCache.add(relPath);
            for (const chain of reExportChains) {
                if (chain.from === relPath) {
                    if (isReachable(chain.to)) return true;
                }
            }
            return false;
        };

        for (const [relPath, b] of this.bindings) {
            const mod = this.modules.get(relPath);
            if (!mod) continue;
            for (const imp of b.imports) {
                const targetRel = this._resolveImportSource(mod, imp.source);
                if (!targetRel) continue;
                if (this.targetModule && targetRel !== this.targetModule) continue;
                if (imp.kind === 'named' && imp.binding === name) {
                    ...
                    const target = this.bindings.get(targetRel);
                    if (target && (target.exports[name] || this.byExport.get(name)?.some((d) => d.module === targetRel))) {
        ...
        const references = [
            ...producers.map((p) => ({ file: p.module, line: p.line, kind: 'producer', type: p.type })),
            ...consumers.map((c) => ({ file: c.module, line: c.line, kind: 'consumer', via: c.via })),
            ...reExportChains.map((c) => ({ file: c.from, line: null, kind: 'reexport', to: c.to }))
        ];

        return { producers, consumers, reExportChains, references };
    }

    _resolveImportSource(mod, source) {
        if (!mod) return null;
        const abs = resolveImportPath(mod.abs, source);
        if (!abs) return null;
        return rarebert.relPath(abs);
    }
}
