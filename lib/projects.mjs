...
/**
 * Human-readable labels for known top-level folders.
 * 
 * These labels are INTENDED FOR USE when the TUI operates on the rarebert
 * project itself (the development environment). When operating on external
 * projects, the TUI will use auto-generated labels from folder discovery.
 * 
 * Unknown folders get a generated label of the form `name/  (auto)`.
 */
const FOLDER_LABELS = {
    lib: 'lib/  (rarebert libraries - for TUI on rarebert project)',
    scripts: 'scripts/  (rarebert core api - for TUI on rarebert project)',
    supports: 'supports/  (rarebert language support - for TUI on rarebert project)',
    src: 'src/  (rarebert projects - for TUI on rarebert project)',
    docs: 'docs/  (rarebert documentation - for TUI on rarebert project)'
};
...
class Project {
     ...
     * 
     * Note: The root label is for use when TUI operates on a registered
     * external project (rarebert folder), not on rarebert's own install.
     */
    _foldersFromRegistered(rows) {
        const folders = [
            new ProjectFolder({
                ...
                rel: '.',
                dir: this.root,
                label: './  (external project root - for registered projects)',
                exts: ['.mjs', '.js']
            })
        ];
        for (const r of rows) {
    ...
    _scanFolders() {
        ...
        // Note: This label is for use when TUI operates on rarebert's own install
        // (home project). External projects use registered folders from DB.
        folders.push(
            new ProjectFolder({
                key: 'root',
                rel: '.',
                dir: this.root,
                label: './  (rarebert core - for TUI on rarebert install)',
                exts: ['.mjs', '.js']
            })
        );
     ...
     *
     * IMPORTANT: Folder labels have different meanings depending on context:
     * - For rarebert's own install (home project): labels indicate rarebert-specific folders
     * - For external projects (rarebert folder): labels show registered project folders
     * - For external projects (other): labels are auto-generated or user-registered
     *
