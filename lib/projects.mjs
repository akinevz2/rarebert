...
class Project {
    constructor(root, { cached = false } = {}) {
        this.root = root;
        // `rarebert` (rarebert's own install) is immutable during a process,
        // so its folders are cached at construction. External projects
        // (`current`) re-evaluate on every discover() call so onboarding
     ...
     * rarebert's own install (rarebert) and as a pre-onboarding default.
     */
    _loadFolders() {
        try {
     ...
     *
     * For external projects (`current`), this is lazily evaluated on
     * every call so onboarding changes are visible without a reload.
     * For `rarebert` (rarebert's install), folders are cached at construction.
     */
    discover() {
    ...
    /**
     * Redirect this Project to a new root path. Used by `current --core`
     * to point the `current` singleton at the install prefix (rarebert.root)
    ...
    discoverModules(dir, exts) {
        // No dir: default to the scripts folder (runnable modules).
        if (dir === undefined || dir === null) {
            const scriptsFolder = this.projectByKey('scripts');
            if (scriptsFolder) return this.discoverModules(scriptsFolder.dir, scriptsFolder.exts);
            return [];
        }
        ...
        return fs
            .readdirSync(dir)
            .filter((f) => effectiveExts.some((ext) => f.endsWith(ext)))
            .map((f) => {
...
/**
 * `rarebert` is always the rarebert installation directory (where lib/,
 ...
 *
 * `current` is the current working directory projected as a Project.
 ...
 * `current.root === rarebert.root` and the two are interchangeable.
 */
export const rarebert = new Project(ROOT, { cached: true });
