...
class Project {
    constructor(root) {
        this.root = root;
        this._folders = this._scanFolders();
    }
    ...
    _scanFolders() {
        ...
        for (const { parent, name } of NESTED_FOLDERS) {
            const dir = path.join(this.root, parent, name);
            if (!fs.existsSync(dir)) continue;
            folders.push(
                new ProjectFolder({
                    key: name,
                    rel: path.relative(this.root, dir),
                    dir,
                    label: FOLDER_LABELS[name] ?? `${name}/  (auto)`,
                    exts: FOLDER_EXT_DEFAULTS[name] ?? ['.js']
                })
            );
        }
     ...
     * to `modules.mjs`'s Module constructor.
     */
    discover() {
    ...
    discoverModules(dir, exts) {
        // No dir: aggregate modules across all discovered folders.
        if (dir === undefined || dir === null) {
            const all = [];
            for (const folder of this.discover()) {
                all.push(...this.discoverModules(folder.dir, folder.exts));
            }
            return all;
        }
