...
import { AbortError, store } from './core.mjs';
...
class Project {
     ...
     * module.
     *
     * Backs the root Dispatcher (index.js) and the Interface/Module
     * machinery.
     */
    resolveModuleRef(ref) {
        const scripts = this.discoverModules();
        const isPathRef = ref.includes('/') || ref.includes(path.sep) || ref.startsWith('./');

        if (isPathRef) {
            const rel = this.relPath(path.resolve(this.root, ref));
            const found = scripts.find((s) => s.path === rel || s.path === ref);
            return found || { name: path.basename(ref, path.extname(ref)), path: rel };
        }

        const name = this.normalizeModuleName(ref);
        return scripts.find((s) => this.normalizeModuleName(s.name) === name) || null;
    }
