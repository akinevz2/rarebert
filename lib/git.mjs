...
class Git {
    ...
    async commitFlow(rel) {
        ...
        // Reached only from the interactive edit flow (scripts/edit.mjs is a
        // TUI module), so Interface construction is safe here.
        const iface = Interface.createInterface('git');
        ...
        if (action === 'discard') {
            const ok = await iface.confirm(`Discard changes to ${rel}? This is destructive.`, false);
            if (!ok) return 0;
