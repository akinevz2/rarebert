...
import { listAllModules, resolveModule, cli } from './module.mjs';
...
async function printIntelligence(relPath, content, ext) {
    ...
    if (mainFunc) {
        console.log('\n--- main() ---');
        console.log(`  ${relPath}:${mainFunc.startLine}-${mainFunc.endLine} (+${mainFunc.bodyLines.length} lines)`);
    } else {
        console.log('\n--- main() ---');
...
async function runDocumentationPass(relPath, content, ext, { verbose, yes, model }) {
    ...
    if (yes) {
        ...
        const confirmed = await cli.confirm(`Memoize ${blockCount} block(s) of documentation to ${relPath}?`, false);
        if (confirmed) {
            memoize();
            memoized = true;
        }
