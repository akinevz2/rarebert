 ...
 */
class ExitSignal {
    constructor(exitCode = 0, producedValue = undefined, onExit = undefined) {
    ...
    async complete() {
        if (typeof this.onExit === 'function') {
            const callbackResult = await this.onExit(this.producedValue);
            if (callbackResult && typeof callbackResult.execute === 'function') {
                return callbackResult;
            }
        }
        return { exitCode: this.exitCode, producedValue: this.producedValue };
    }
}
...
class Runtime {
    ...
    async execute(args = []) {
        ...
        // code smell: not the best pattern.
        // optimistically - args and positionals should be parsed first
        // and should not be reset to empty when executing a completed module
        for (;;) {
            ...
            // handle case where completed is object?
            const { exitCode, producedValue } = completed;

            if (producedValue !== undefined && producedValue !== null) {
                if (exitCode === 0) {
                    // dir should be used when produced value exists unconditionally
                    console.dir(producedValue);
                } else {
                    // error should be used if exitCode is non zero unconditionally
                    console.error(producedValue);
                }
            }

            // if produced value is non null, it should be returned rather than exitCode
            // otherwise exitCode should be returned
            return exitCode;
        }
    }
}
...
function exit(exitCodeOrValue, exitResult) {
    ...
    if (typeof exitCodeOrValue === 'number') {
        ...
        const producedValue = onExit ? undefined : exitResult;
        return new ExitSignal(exitCodeOrValue, producedValue, onExit);
    }
...
export { ExitSignal, HelpRequestedSignal, Runtime, exit, EXIT_OK, EXIT_FAIL, EXIT_ABORT, Store, store, DB_PATH, DATA_DIR };
export default { ExitSignal, HelpRequestedSignal, Runtime, exit, EXIT_OK, EXIT_FAIL, EXIT_ABORT, Store, store, DB_PATH, DATA_DIR };
