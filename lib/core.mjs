...
/**
 * ExitSignal represents the result of a module execution. It composes
 * four kinds, resolved by complete() in this order:
 *
 *   promise   — a thenable held in `submodule`. Awaited; its outcome folds
 *               back through the machinery (ExitSignal → recurse, number →
 *               code, rejection → error kind).
 *   submodule — a Module instance (runs its own execute() and folds the
 *               returned signal/code) or a plain function (invoked).
 *   error     — an Error held in `error`. Reports the message (unless a
 *               custom onExit is attached), runs cleanup + onExit, and
 *               yields `error.exitCode` (AbortError → 130) or EXIT_FAIL.
 *   code      — a plain exit code with optional cleanup + onExit.
 *
 * Because every kind funnels through complete(), a main callback can
 * `return exit(<any kind>)` and Module.exit() handles continuation
 * uniformly — no ad-hoc process.exit() branches at callsites.
 *
...
class ExitSignal {
    ...
    async complete() {
        // Promise kind — await the thenable and fold its outcome back into
        // the signal machinery.
        if (this.isPromise()) {
            let result;
            try {
                result = await this.submodule;
            } catch (err) {
                return await ExitSignal.fromError(err, {
                    onExit: this.onExit,
                    retry: this.retry,
                    cleanup: this.cleanup
                }).complete();
            }
            if (result instanceof ExitSignal) return await result.complete();
        ...
        // Submodule kind — a Module instance runs its own execute() and
        // folds the returned signal/code.
        if (this.submodule && typeof this.submodule.execute === 'function') {
            try {
                const result = await this.submodule.execute();
                if (result === undefined) {
                    console.error(
                        `${this.submodule.path || 'submodule'}: main callback returned undefined — use exit() to signal completion.`
                    );
                    if (typeof this.onExit === 'function') await this.onExit(1);
                    return 1;
                }
                const exitCode =
                    result instanceof ExitSignal
        ...
        // Submodule kind — a plain function is invoked and its numeric
        // result (if any) becomes the code.
        if (typeof this.submodule === 'function') {
            try {
        ...
        // Error kind — report through the machinery: AbortError maps to its
        // own exitCode (130); anything else fails with EXIT_FAIL. cleanup
        // and onExit still run; with no onExit the message prints to stderr.
        if (this.error) {
            const code = this.error.exitCode ?? this.code ?? EXIT_FAIL;
            if (this.cleanup) {
                try {
                    await this.cleanup();
                } catch {
            ...
            if (typeof this.onExit === 'function') {
                ...
                console.error(this.error.message || String(this.error));
            }
            return code;
        }
    ...
    isPromise() {
        return !!this.submodule && typeof this.submodule.then === 'function';
    }
    ...
    static fromError(err, { onExit, retry, cleanup } = {}) {
        return new ExitSignal(err?.exitCode ?? EXIT_FAIL, onExit, retry, cleanup, null, err);
    }

    static fromPromise(promise, { onExit, retry, cleanup } = {}) {
        return new ExitSignal(EXIT_OK, onExit, retry, cleanup, promise);
    }
}
...
// AbortError — thrown on ctrl-c / escape from Enquirer prompts. Lives here
// (the signal home, which imports nothing from lib/) so low-level modules
// like projects.mjs can use it without importing module.mjs — breaking the
// projects ↔ module import cycle. module.mjs re-exports it for compat.
...
function exit(codeOrSubmodule, options = {}) {
    // Antipattern guard: exit(n, fn) / exit(n, runnable) used to silently
    // drop the second argument (it was read as `options`), so the callback
    // or submodule never ran. Fail fast with guidance instead — submodules
    // are the FIRST argument (exit(fn) / exit(flow)); callbacks belong in
    // the { onExit } options object.
    if (
        typeof options === 'function' ||
        (options && typeof options === 'object' && typeof options.execute === 'function')
    ) {
        throw new TypeError(
            'exit(code, fn) is not supported — the second argument would be silently dropped. ' +
                'Use exit(fn) / exit(flow) for submodules, or exit(code, { onExit: fn }) for callbacks.'
        );
    }
    const { onExit, retry, cleanup } = options;

    // Error kind — the error rides the signal; AbortError maps to its own
    // exitCode (130), anything else fails with EXIT_FAIL.
    if (codeOrSubmodule instanceof Error) {
        return ExitSignal.fromError(codeOrSubmodule, { onExit, retry, cleanup });
    }

    // Promise kind — the awaited outcome folds back through complete():
    // ExitSignal → recurse, number → code, rejection → error kind.
    if (codeOrSubmodule && typeof codeOrSubmodule.then === 'function') {
        return ExitSignal.fromPromise(codeOrSubmodule, { onExit, retry, cleanup });
    }
    ...
    if (typeof codeOrSubmodule === 'string') {
        return new ExitSignal(
            1,
            onExit ?? (() => console.error(codeOrSubmodule)),
            retry,
            cleanup,
            null
        );
...
class Store {
    ...
    db() {
        ...
        const tables = this._db
            .prepare(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='introspect_cache'"
            )
            .all();
        if (tables.length === 0) {
