        ...
        if (this.submodule) {
            try {
                if (typeof this.submodule.execute === 'function') {
                    ...
                    if (result === undefined) {
                        console.error(`${this.submodule.path || 'submodule'}: main callback returned undefined — use exit() to signal completion.`);
...
function exit(codeOrSubmodule, options = {}) {
    ...
    if (typeof codeOrSubmodule === 'string') {
        return new ExitSignal(1, onExit ?? (() => console.error(codeOrSubmodule)), retry, cleanup, null);
    }
...
class Store {
    ...
    /** Return all registered projects as [{ id, path, onboarded, editor_type, last_model, created_at }] or []. */
    listProjects() {
