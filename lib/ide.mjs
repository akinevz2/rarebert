 ...
 * stdio/await strategy based on the persisted editor-type
...
class Ide {
    ...
    /**
     * True when the persisted editor-type preference is
    ...
    /**
     * Run an opencode headless pass and return stdout. Logs an error
     * to stderr when the underlying spawn exits non-zero, but still
     * returns whatever stdout was captured (possibly empty). Callers
     * that need to branch on exit status should use spawnHeadless
     * directly.
     */
    runOpencodeHeadless(prompt, model) {
        const { status, stdout } = this.spawnHeadless(prompt, model);
        if (status !== 0) {
