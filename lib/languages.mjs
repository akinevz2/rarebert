...
const tui = new TUI('languages.mjs');
...
class Languages {
    ...
    async installNew() {
        ...
        if (this.isSupported(name)) {
            const overwrite = await tui.confirm(`Language "${name}" is already installed. Overwrite?`, false);
    ...
    async installFromArgs(opts = {}, positional = []) {
        ...
        let lang = nameArg;
        if (!lang) {
            if (!cli.isInteractive()) return cli.fail('Usage: node index.js languages install <lang> [--force]');
        ...
        if (this.isSupported(lang) && !force) {
            const overwrite = await tui.confirm(`Language "${lang}" is already installed. Overwrite?`, false);
            if (!overwrite) return cli.ok('Not overwritten.');
            force = true;
        }
