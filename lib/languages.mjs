...
class Languages {
    ...
    /**
     * Resolve a section of the language's boilerplate template into an
     * array of substituted lines. Delegates the pure substitution work
     * to template.mjs; this method only loads the `Language` instance
     * and hands its `Template` to the pure `resolveTemplate` function.
     */
    async resolveTemplate(ext, vars = {}, section = 'module') {
        const lang = await this.loadLanguage(ext);
        return resolveTemplate(lang.template, vars, section);
    }
