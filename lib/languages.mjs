...
import { cli, Interface } from './module.mjs';
...
class Languages {
    ...
    async installNew() {
        // Only reached from choose(), which guards with cli.isInteractive()
        // before prompting.
    ...
    async choose({ default: defaultLang = 'mjs' } = {}) {
        ...
        const choice = await iface.select('Select a language for the new module:', this.choices(), {
    ...
    async installFromArgs(opts = {}, positional = []) {
        ...
        let lang = nameArg;
        // Constructed lazily so the Interface is built once, only after an
        // interactive guard has passed (the confirm path below is reachable
        // non-interactively when a lang arg is given without --force).
        let iface = null;
