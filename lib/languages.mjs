 ...
 *   @param {(content: string) => object|null} [impls.extractEntryPoint]
 *     Optional rarebert-aware entry-point detector. When the module being
 *     analyzed belongs to a rarebert project, this is called instead of
 *     extractMainFunction so that `export default new CLI(...)` and
 *     `export default new TUI(...)` are recognized as entry points.
 */
class Language {
    constructor(
        ext,
        ...
        this.extractEntryPoint = extractEntryPoint;
    }
}
...
class Languages {
     ...
     *  When analyzing a module inside a rarebert project and the language
     *  supports it, also recognizes `export default new CLI(...)` and
     *  `export default new TUI(...)` as entry points (rarebert-specific
     *  grace; non-rarebert projects get standard main() detection only).
     */
    async extractMainFunction(content, ext) {
        const lang = await this.loadLanguage(ext);
        const isRarebert = current.root === rarebert.root;
        if (isRarebert && lang.extractEntryPoint) {
            return lang.extractEntryPoint(content) ?? lang.extractMainFunction(content);
        }
    ...
    async opencodeGenerateTemplate(lang) {
        ...
        const prompt = [
            ...
            `The template must be an object with two fields: "lines" (a map of symbolic line keys to template strings) and "sections" (an array of line keys defining the module layout).`,
            `Use placeholders like {{MODULE_NAME}} and {{LIB_IMPORTS}} where appropriate.`,
            `Mirror the structure used by the existing templates in lib/supports/ (langmjs.js, langjs.js, langpy.js) but adapt the syntax to "${lang}".`,
