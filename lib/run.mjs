...
class ModuleArguments extends Array {
    ...
    /**
     * True if a flag is present. Checks three ways:
     * 1. `flag in this` — direct key match (for merged opts like 'verbose').
     * 2. camelCase key — '--yes' → 'yes' in opts (Commander strips -- and
     *    camelCase-ifies flag names).
     * 3. `this.includes(flag)` — literal `--flag` token in the positional
     *    array (for allowUnknownOption modules like memo).
     */
    has(flag) {
        if (flag in this) return true;
        const key = flag.replace(/^--?/, '').replace(/-([a-z])/g, (_, c) => c.toUpperCase());
        if (key in this._opts && this._opts[key] !== false && this._opts[key] !== undefined) return true;
    ...
    async execute(args = []) {
        for (;;) {
            ...
            if (typeof mod.guard === 'function') {
                const guardResult = await mod.guard(args);
                if (guardResult instanceof ExitSignal) {
                    if (typeof mod.cleanup === 'function') {
                        try { await mod.cleanup(args); } catch { /* never let cleanup throw cascade */ }
                    }
                    const { exitCode, producedResult } = guardResult;
    ...
    static async assertSaneExit(mod, args = []) {
        ...
        try {
            ...
            if (typeof mod.guard === 'function') {
                ...
                if (guardResult instanceof ExitSignal) {
                    report.ok = true;
                    report.exitCode = guardResult.exitCode;
                    report.producedResult = guardResult.producedResult;
                    return report;
                }
            }
            const runner = Runtime.createRunner(mod.meta, mod.main);
