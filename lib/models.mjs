...
import { Interface } from './module.mjs';
...
class Models {
    ...
    async prompt(models, fallback = this.defaultModel()) {
        ...
        // Interface prompt factory — submodule-level runnable, thenable:
        // callers await the value; the exit() machinery can also run it.
        const iface = Interface.createInterface('models');
        return iface.select('Select a model to implement with', choices, {
