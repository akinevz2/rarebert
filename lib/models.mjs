...
class Models {
    ...
    async resolve(arg, env = process.env) {
        ...
        if (arg) {
            ...
            const preferred = lastChosen || def;
            if (preferred && config.model !== preferred) {
                config.model = preferred;
            }
            modelId = await this.prompt(this.list(config), def);
    ...
    /**
     * Read the Modelfile to extract model specifications. Looks for
     * Modelfile.server or Modelfile.refactor in the home directory.
     * Returns the model id if found, or null if no Modelfile exists.
     */
    async fromModelfile() {
        const modelfilePaths = [
            path.join(home.root, 'Modelfile.server'),
            path.join(home.root, 'Modelfile.refactor'),
            path.join(home.root, 'Modelfile')
        ];
        
        for (const modelfile of modelfilePaths) {
            if (fs.existsSync(modelfile)) {
                try {
                    const content = fs.readFileSync(modelfile, 'utf-8');
                    // Look for model specification in Modelfile
                    // Common patterns: FROM provider/model, PARAMETER model value
                    const fromMatch = content.match(/^FROM\s+(\S+)/m);
                    if (fromMatch) return fromMatch[1];
                    
                    const paramMatch = content.match(/PARAMETER\s+model\s+(\S+)/i);
                    if (paramMatch) return paramMatch[1];
                    
                    // Look for model in FROM line (e.g., FROM ollama/model-name)
                    const fromLine = content.split('\n').find(line => line.startsWith('FROM'));
                    if (fromLine) {
                        const parts = fromLine.split(/\s+/);
                        if (parts.length > 1) return parts[1];
                    }
                } catch {
                    // Continue to next Modelfile if this one fails
                }
            }
        }
