...
const OPENCODE_CONFIGS = ['opencode.jsonc', 'opencode.json'].map((f) => path.join(rarebert.root, f));
const DEFAULT_MODEL = 'ollama/glm-5.2:cloud';

class Models {
    constructor() {
        this.configPaths = OPENCODE_CONFIGS;
    ...
    async prompt(models, fallback = this.defaultModel) {
        if (models.length === 0) {
            console.log(`No models found in opencode.jsonc; using fallback: ${fallback}`);
            return fallback;
        }
    ...
    /**
     * Write `modelId` back to opencode.jsonc as the default model. Silently
