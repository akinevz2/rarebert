 ...
 * Construct from a constituent descriptor (see `rarebert.discover()`) and
 * the module filename; the absolute path is derived, never supplied.
 */
class Module {
...
function listAllModules() {
    ...
    for (const project of rarebert.discover()) {
        for (const m of rarebert.discoverModules(project.dir, project.exts)) {
            modules.push(new Module(project, path.basename(m.path)));
        }
    }
    return modules;
}
