...
const mjsLines = {
    ...
    spec_note: [
        '// {{MODULE_NAME}}.mjs — scaffolded by `make add`.',
        '//',
        '// Implement against the Module authoring specification in AGENTS.md:',
        '//   - main(opts, positional): opts is an assured Arguments instance',
        '//     (Commander-backed — positionals with opts merged, .has(flag), .help()).',
        '//   - return exit(0) | exit(n) | exit("explanation") | exit(err) — never',
        '//     process.exit(); the Runtime (lib/runtime.mjs) owns termination.',
        '//   - declare flags/positionals in meta (options/args) — Commander owns',
        '//     parsing and required-argument validation.',
        '//   - interactive work: TUI member prompts or Interface.createInterface()',
        '//     flows; runInteractively() bails non-interactive invocations.'
    ].join('\n'),
    blank: '',
    imports: "import { CLI } from '{{CLI_IMPORT}}'; // TUI, Interface, cli also exported there",
    ...
    main_scaffold:
        '    // {{MODULE_NAME}}: implement per the Module authoring specification (AGENTS.md)',
