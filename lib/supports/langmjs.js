 ...
 *
 * For (2)/(3), the arrow function passed as the second constructor
...
const mjsExtractEntryPoint = (content) => {
    ...
    // Find `export default new CLI(` or `export default new TUI(`.
    let ctorLine = -1;
    ...
    if (ctorLine === -1) return null;

    // Find the arrow function `async (...) => {` within the constructor
    // call (may be on the same line or a subsequent line).
    let arrowLine = -1;
    for (let i = ctorLine; i < Math.min(ctorLine + 5, lines.length); i++) {
    ...
    // Extract the arrow body using brace counting, starting from the
    // opening `{` of the arrow body.
    let braceCount = 0;
    let foundOpen = false;
    let bodyStart = -1;
    let bodyEnd = -1;
    ...
    // Trim blank lines at the edges of the body.
