...
const jsExtractBindings = (content) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (moduleExportsObj) {
            ...
            const keys = objText.replace(/^module\.exports\s*=\s*{/, '').replace(/}\s*;?\s*$/, '');
            const pairs = keys
                .split(',')
                .map((s) => s.trim())
                .filter(Boolean);
        ...
        const moduleExportsFn =
            isTopLevel && trimmed.match(/^module\.exports\s*=\s*(?:function|class|\w)/);
        ...
        if (exportsObj) {
            ...
            const keys = objText.replace(/^exports\s*=\s*{/, '').replace(/}\s*;?\s*$/, '');
            const pairs = keys
                .split(',')
                .map((s) => s.trim())
                .filter(Boolean);
