...
`;
    ...
    runOnServer({ url = DEFAULT_HOST, port = DEFAULT_PORT, prompt, model = null, auto = false } = {}) {
        ...
        if (!running && attachToLocal) {
            ...
            return {
                status: this.startFullTUI({ cwd: rarebert.root, model, port, prompt: prompt ?? null }),
                stdout: ''
            };
        }
        ...
        const bin = opencode.resolve();
        const args = ['run', prompt, '--attach', attachUrl, '-u', 'opencode', '-p', String(attachPort)];
