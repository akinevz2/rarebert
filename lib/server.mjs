// ---------------------------------------------------------------------------
// OUTDATED / DEPRECATED for HTTP server purposes.
//
// This module manages the **opencode process lifecycle** (spawn, port
// probing, server info files). It is NOT an HTTP server and should not
// be used for the OpenAI-compatible server feature.
//
// The OpenAI-compatible HTTP server with programmable routing middleware
// lives in `src/server/` (TypeScript). See:
//   - docs/server-architecture.md
//   - docs/middleware.md
//
// This module is still used by `lib/ide.mjs` and `lib/implement.mjs`
// for opencode TUI/headless spawning. Do not remove it.
// ---------------------------------------------------------------------------
...
import { stripJsonc, models } from './models.mjs';

// REQUEST: startFullTUI and spawnTui functions spawn opencode processes that may need cleanup
// on ctrl-c. On abort:
// - Kill any spawned opencode child processes
// - Clear the server info file (.opencode/server)
// Meta suggestion: { retryOnFailure: false, cleanup: 'killServerProcess' }
...
function configFingerprint(root) {
    ...
    for (const rel of tryPaths) {
        ...
        if (fs.existsSync(abs)) {
            try {
                // opencode.jsonc permits JSONC syntax — strip comments
                // and trailing commas before JSON.parse.
                cfg = JSON.parse(stripJsonc(fs.readFileSync(abs, 'utf-8')));
            } catch {
...
`;
    ...
    startFullTUI({ cwd, model, port = null, prompt = null } = {}) {
        const err = models.validateModel(model);
        if (err) {
            console.error(`implement: model error — ${err}`);
