...
import { isSupported, listLanguages } from './languages.mjs';
...
const getExtension = (name) => libs.getExtension(name);
const projectLibDir = (lang) => libs.projectLibDir(lang);
const findLibraries = (libDir, ext) => libs.findLibraries(libDir, ext);
const findProjectLibs = (lang) => libs.findProjectLibs(lang);
const findAllProjectLibs = () => libs.findAllProjectLibs();
const createModule = (directory, moduleName, ext) => libs.createModule(directory, moduleName, ext);
const dirForDirectory = (directory) => libs.dirForDirectory(directory);
const relPath = (absPath) => libs.relPath(absPath);
