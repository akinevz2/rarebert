...
import { DIAMOND, GREEN_TICK } from '../scripts/symbols.mjs';
...
function presentSlide(slide) {
    ...
    const filePath = path.isAbsolute(slide.file)
        ? slide.file
        : path.join(current.root, slide.file);
...
async function buildPresentation(instruction, model, opts) {
    ...
    const { status, stdout } = ide.spawnHeadless(prompt, model, { cwd: current.root });
    if (status !== 0) {
