...
const readLastModule = () => editor.readLastModule();
const writeLastModule = (rel) => editor.writeLastModule(rel);
const clearLastModule = () => editor.clearLastModule();
const editFile = (fp) => editor.editFile(fp);
const loadContent = (f) => editor.loadContent(f);
const loadStageContent = (f) => editor.loadStageContent(f);
const resolveTargetArg = (a) => editor.resolveTargetArg(a);
const resolveActiveFiles = (args, opts) => editor.resolveActiveFiles(args, opts);
