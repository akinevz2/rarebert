...
const normalizeModuleName = (rawName) => current.normalizeModuleName(rawName);

const LAST_MODULE_FILE = path.join(rarebert.root, '.last-module');
...
class Editor {
    ...
    loadContent(file) {
        const abs = path.isAbsolute(file) ? file : path.join(current.root, file);
        if (!fs.existsSync(abs)) return null;
        const rel = path.relative(current.root, abs) || file;
        const body = fs.readFileSync(abs, 'utf-8');
        return `--- ${rel} ---\n${body}`;
    }

    loadStageContent(file) {
        const rel = path.isAbsolute(file) ? path.relative(current.root, file) : file;
        const header = `--- ${rel} (tracked + modified; staged patch) ---`;
        const patch = git.stagedDiffForPath(rel) || '';
        const body = fs.existsSync(path.join(current.root, rel))
            ? fs.readFileSync(path.join(current.root, rel), 'utf-8')
            : '';
    ...
    resolveTargetArg(arg) {
        const abs = path.isAbsolute(arg) ? arg : path.join(current.root, arg);
    ...
    moduleChoices(modules) {
        return modules.map((s) => {
            const meta = current.getScriptMetadata(s.path);
            const desc = meta.description ? meta.description.split('\n')[0].trim() : '';
            const rel = path.relative(current.root, s.path);
    ...
    async promptWorkspaceMultiSelect(dir) {
        ...
        if (files.length === 0) {
            console.error(`editor: no files in ${path.relative(current.root, dir) || '.'}`);
            return [];
        }
        const choices = files.map((f) => {
            const rel = path.relative(current.root, f);
    ...
    async promptWorkspaceChoice() {
        const choices = current.discover().map((d) => ({
            name: d.key,
            message: d.label
        }));
        ...
        while (!confirmed) {
            ...
            while (true) {
                ...
                for (const p of picked) {
                    const rel = path.relative(current.root, p);
