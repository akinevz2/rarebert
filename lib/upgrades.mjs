...
function categorize(rows) {
    ...
    for (const row of rows) {
        switch (row.op) {
            case 'A': buckets.added.push(row); break;
            case 'M': buckets.modified.push(row); break;
            case 'D': buckets.deleted.push(row); break;
            case 'R': buckets.renamed.push(row); break;
            case 'C': buckets.copied.push(row); break;
            default: buckets.other.push(row); break;
        }
    }
    return buckets;
}
...
function extractMethods(source, ext) {
    ...
    const patterns = [
        ...
        { re: /export\s+const\s+(\w+)\s*=\s*(?:async\s*)?\(/g, kind: 'const arrow', exported: true },
        { re: /export\s+default\s+async\s+function\s+(\w+)\s*\(/g, kind: 'async function (default)', exported: true },
        { re: /export\s+default\s+function\s+(\w+)\s*\(/g, kind: 'function (default)', exported: true },
    ...
    const KEYWORD_METHOD_NAMES = new Set(['if', 'for', 'while', 'switch', 'catch', 'return', 'do', 'else', 'try', 'finally', 'function', 'async']);
    const seen = new Set();
...
function printSummary(buckets, inventory, base) {
    ...
    const total =
        buckets.added.length + buckets.modified.length + buckets.deleted.length +
        buckets.renamed.length + buckets.copied.length + buckets.other.length;
