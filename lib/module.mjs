...
class Module {
    ...
    createCommand(meta) {
        ...
        // Commander-native positional-argument declarations: meta.args
        // entries become program.argument() declarations so Commander owns
        // required-argument checking (replacing the hand-rolled
        // _validateArgs pass — a missing required arg now surfaces as a
        // CommanderError through _parseArgv and folds via exit(err)).
        for (const spec of meta.args || []) {
            program.argument(
...
// ---------------------------------------------------------------------------
// Arguments — the assured arguments object handed to every main callback.
//
// An Array subclass (the positional args) with the parsed Commander opts
// merged onto the instance, so legacy `main(opts, positional)` property
// access (`args.debug`) keeps working while every main is guaranteed an
// instance of Arguments and can use Commander's own conveniences:
// `args instanceof Arguments`, `args.command` (the Command instance),
// `args.opts`, `args.has(flag)`, `args.help()`.
// ---------------------------------------------------------------------------

class Arguments extends Array {
    static from(command, positional) {
        const inst = super.from(Array.isArray(positional) ? positional : []);
        inst.command = command;
        Object.assign(inst, command.opts());
        return inst;
    }

    /** The parsed Commander opts object. */
    get opts() {
        return this.command.opts();
    }

    /** Raw-argv presence check for a flag (works for undeclared flags). */
    has(flag) {
        return this.command.rawArgs?.includes(flag) ?? false;
    }

    /** Commander's rendered help for this module. */
    help() {
...
class CLI extends Module {
    ...
    _buildActionHandler(program, meta, main, capture) {
        return async () => {
            const positional = program.args.slice();
            try {
                // Every main receives an assured Arguments instance (the
                // positional args with Commander opts merged) — legacy
                // (opts, positional) mains keep working via the merge.
                capture(await main(Arguments.from(program, positional), positional));
            } catch (err) {
...
// contains class definitions only, and lib/tui.mjs is deleted. All prompt
// call sites use the Interface prompt factories
// (Interface.createInterface(...).confirm/input/select/query).
