...
class _SharedCLI {
    ...
    /**
     * DEPRECATED: die() bypasses the exit() callback system.
     * Use exit(code, { onExit: () => {} }) instead.
     * This method throws an error to force migration to exit().
     */
    die(message, code = EXIT_FAIL) {
        if (message) {
        ...
        this.clearCtrlcTimer();
        this.runAbortCallbacks();
        throw new Error(`die() called with code ${code} - use exit() instead`);
    }

    /**
     * DEPRECATED: abort() uses die() internally.
     * Use exit(130, { onExit: () => {} }) or throw AbortError instead.
     */
    abort(message = '\nAborted.') {
        this.die(message, 130);
    }

    /**
     * DEPRECATED: ok() uses die() internally.
     * Use exit(0) instead.
     */
    ok(message) {
    ...
    /**
     * DEPRECATED: fail() uses die() internally.
     * Use exit(1) instead.
     */
    fail(message) {
    ...
    /**
     * DEPRECATED: nonInteractive() calls process.exit() directly.
     * This bypasses the exit() callback system.
     * Use exit(code, { onExit: () => {} }) instead.
     */
    nonInteractive(message, code = EXIT_FAIL) {
