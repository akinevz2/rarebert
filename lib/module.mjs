...
import { current, rarebert } from './projects.mjs';
...
class Module {
    constructor(file, meta, main) {
        // Auto-detect the project folder by matching the file basename
        // against rarebert's own install folders (rarebert), since scripts
        ...
        this.ext = ext;
        this.main = main || null;
        this.altMode = null; // TUI for CLI (or CLI fallback for TUI)
    ...
    /**
     * Execute the module and handle its result. Convenience wrapper
     * around execute() + exit() for callers that want both.
     */
    ...
    static fromProject(project, file) {
        ...
        mod.path = current.relPath(mod.abs);
        mod.dir = project.rel;
        mod.meta = null;
        mod.main = null;
        return mod;
    ...
    supportsDirectRunning(metaUrl) {
        ...
        if ((argv1 && resolved === argv1) || (realArgv1 && resolved === realArgv1)) {
            ...
            this.executeAndExit(process.argv.slice(2));
        }
        return this;
    }
}
...
class CLI extends Module {
    constructor(file, meta, main) {
        // Backward compatibility: old signature (file, mainFn, meta)
        if (typeof main === 'object' && typeof meta !== 'object') {
            [meta, main] = [main, meta];
        }

        super(file, meta, main);
        // Wrap main in a Commander runner so this.executeAndExit(args) parses argv
        // before calling the user's main callback.
        this.main = main ? this._wrap(this.meta, main) : null;
    }
    ...
    die(message, code = EXIT_FAIL) {
        return sharedCLI.die(message, code);
    }

    abort(message = '\nAborted.') {
        return sharedCLI.abort(message);
    }

    ok(message) {
        return sharedCLI.ok(message);
    }
...
function findDirectoryTarget(key) {
    return current.discover().find((t) => t.key === key) || null;
}

function directoryTargetByPath(absPath) {
    const resolved = path.resolve(absPath);
    const targets = current.discover();
    return (
...
function listAllModules() {
    const modules = [];
    for (const project of current.discover()) {
        for (const m of current.discoverModules(project.dir, project.exts)) {
...
function buildModuleChoices(modules) {
    return modules.map((s) => {
        const meta = current.getScriptMetadata(s.abs);
...
function resolveModule(arg, modules) {
    const rel = path.isAbsolute(arg) ? current.relPath(arg) : arg;
    const mod =
...
function resolveModuleSet(args, modules) {
    ...
    for (const arg of args) {
        const abs = path.isAbsolute(arg) ? arg : path.resolve(current.root, arg);
...
async function promptModule(modules, moduleArg, message = 'Select a module') {
    if (moduleArg) {
        const match =
            modules.find(
                (s) =>
                    current.normalizeModuleName(s.name) === current.normalizeModuleName(moduleArg)
            ) ||
