...
class Module {
    ...
    /**
     * Pre-run guard hook — Runtime calls this before invoking the runner.
     * Default is a no-op; TUI overrides to reject non-interactive mode.
     * Return a rejected promise or exit('...') to abort.
     */
    async guard(_args) {}

    /**
     * Post-run cleanup hook — Runtime calls this after a terminal
     * ExitSignal (not when looping to a produced Module). Default no-op;
     * TUI overrides to clear the screen.
     */
    async cleanup(_args) {}
    ...
    createCommand(meta) {
        return Runtime.createCommand(meta);
    }

    wantsHelp(args) {
        return Runtime.wantsHelp(args);
    }
...
// interfaces. TUI inherits CLI's runner (Commander arg parsing) and overrides
// guard()/cleanup() — Runtime calls guard before invoking the runner (TUI
// rejects non-interactive mode there) and cleanup after a terminal result
// (TUI clears the screen). The runner itself is unchanged from CLI.
// ---------------------------------------------------------------------------
...
class TUI extends CLI {
    ...
    /**
     * Pre-run guard: reject non-interactive mode before the runner fires.
     * Runtime calls this before invoking module.runner.
     */
    async guard(_args) {
        if (!this.isInteractive()) {
            ...
            return exit(`tui: ${this.path} requires an interactive terminal (stdin is not a TTY).`);
        }
    }

    /**
     * Post-run cleanup: clear the screen after a terminal ExitSignal so
     * the next shell line doesn't clash with TUI output. Runtime calls
     * this only when NOT looping to a produced Module.
     */
    async cleanup(_args) {
        this.clear();
    }
...
async function promptModule(modules, moduleArg, message = 'Select a module') {
    ...
    if (process.stdin.isTTY !== true) {
        return exit('Non-interactive; pass a module name as an argument.');
    }
