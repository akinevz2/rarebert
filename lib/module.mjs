...
class Module {
    ...
    async executeAndExit(args = []) {
        ...
        for (;;) {
            const result = await current.execute(currentArgs);
            const outcome = await current.exit(result);
            if (outcome && typeof outcome.execute === 'function') {
                current = outcome;
                currentArgs = [];
                continue;
            }
    ...
    supportsDirectRunning(metaUrl) {
        ...
        if ((argv1 && resolved === argv1) || (realArgv1 && resolved === realArgv1)) {
            ...
            this.executeAndExit(process.argv.slice(2)).then((code) => {
