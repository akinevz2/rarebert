...
class CLI extends Module {
    constructor(file, main, meta) {
        ...
        this.runner = main ? Runtime.createRunner(meta, main) : null;
    }
    ...
    /**
     * Run the CLI module. Delegates to Runtime.createRunner (which
     * parses argv via Commander, intercepts --help, and invokes the
     * wrapped main with (opts, positional)). Returns an ExitSignal.
     */
    async execute(args = []) {
        const runner = this.runner || Runtime.createRunner(this.meta, this.main);
        return await runner(args);
    }
}
