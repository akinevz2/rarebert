...
import { ExitSignal, exit } from './core.mjs';
...
class Module {
    ...
    async execute(args = []) {
        ...
        if (!runner) {
            ...
            return exit(1, { onExit: () => console.error(`${this.path}: module has no main callback`) });
        }
        return await runner(args);
    }

    /**
     * Handle the result of execute(). If it is an ExitSignal, runs its
     * onExit hook (via complete()) and exits the process with its code.
     * For submodules, awaits the submodule execution before exiting.
     * 
     * NOTE: This method calls process.exit() to terminate the process.
     * This is the final exit point - all exit signals funnel through here.
     */
    async exit(result) {
        if (result === undefined) {
            console.error(`${this.path}: main callback returned undefined — use exit() to signal completion.`);
            process.exit(1);
        }
        ...
        if (!(result instanceof ExitSignal)) {
            console.error(`${this.path}: main callback returned ${typeof result} — expected an ExitSignal from exit().`);
            process.exit(1);
        }
        ...
        if (result.isSubmodule()) {
            ...
            if (submodule && submodule.clearScreen) {
    ...
    supportsDirectRunning(metaUrl) {
        ...
        if ((argv1 && resolved === argv1) || (realArgv1 && resolved === realArgv1)) {
            ...
            this.executeAndExit(process.argv.slice(2)).catch((err) => {
...
class CLI extends Module {
    ...
    _wrap(meta, main) {
        return async (args = []) => {
            ...
            program.action(this._buildActionHandler(program, meta, main, (r) => { actionResult = r; }));
    ...
    _buildActionHandler(program, meta, main, capture) {
        return async () => {
            ...
            try {
                ...
                if (validationErr) { capture(validationErr); return; }
                capture(await main(opts, positional));
            } catch (err) {
                if (err && err.name === 'AbortError') throw err;
                capture(exit(1, { onExit: () => console.error(err?.message || String(err)) }));
            }
        };
    }
    ...
    async _parseArgv(program, meta, args) {
        try {
            ...
            throw exit(1, { onExit: () => console.error(err?.message || String(err)) });
        }
    }
}
...
class TUI extends CLI {
    ...
    async execute(args = []) {
        if (!this.isInteractive()) {
            console.error(`tui: ${this.path} requires an interactive terminal (stdin is not a TTY).`);
            return exit(1);
        }
...
const cli = {
    ...
    runAbortCallbacks() {
        for (const cb of this.abortCallbacks) {
            try { cb(); } catch { /* never let cleanup throw cascade */ }
        }
    },
    ...
    handleCtrlC() {
        ...
        this.clearCtrlcTimer();
        this.ctrlcTimer = setTimeout(() => { this.ctrlcCount = 0; }, 1000);
        this.runAbortCallbacks();
    },
    ...
    installSignalHandlers() {
        ...
        const handler = () => {
            if (this.aborting) {
                this.ctrlcCount++;
                if (this.ctrlcCount >= 2) { this.clearCtrlcTimer(); process.exit(130); }
                return;
            }
            this.handleCtrlC();
        };
    ...
    isInteractive() { return process.stdin.isTTY === true; },
    ...
    async confirm(message, initial = false) {
        ...
        try { return await prompt.run(); } catch { throw new AbortError(); }
    },
    ...
    async input(message, options = {}) {
        ...
        const prompt = new Input({ name: 'input', message, initial, validate });
        try { return (await prompt.run())?.trim() || initial; } catch { throw new AbortError(); }
    },

    async select(message, choices, options = {}) {
        const { initial = 0, limit = 10, nonInteractiveBehavior = 'fail' } = options;
        if (!this.isInteractive()) {
            if (nonInteractiveBehavior === 'fail') this.nonInteractive('cannot prompt for selection.');
            const choice = choices[initial];
            return typeof choice === 'string' ? choice : choice.name;
        }
        const { Select } = await import('enquirer');
        const prompt = new Select({ name: 'select', message, choices, initial, limit });
        try { return await prompt.run(); } catch { throw new AbortError(); }
    },
    ...
    _typeParser(opt) {
        if (opt.type === 'int') {
            return (v) => { const n = parseInt(v, 10); if (isNaN(n)) throw new Error(`Must be an integer`); return n; };
        }
        if (opt.type === 'float') {
            return (v) => { const n = parseFloat(v); if (isNaN(n)) throw new Error(`Must be a number`); return n; };
        }
        if (opt.type === 'array') {
            return (v, prev = []) => v.includes(',') ? v.split(',').map(s => s.trim()) : [...prev, v];
        }
        if (opt.choices) {
            const choices = new Set(opt.choices);
            return (v) => { if (!choices.has(v)) throw new Error(`Must be one of: ${opt.choices.join(', ')}`); return v; };
        }
        const flagStr = this._flagString(opt);
    ...
    wantsHelp(args) { return args.includes('--help') || args.includes('-h'); },
    ...
    parse(argv = [], options = []) {
        ...
        for (const opt of options) {
            ...
            const def = opt.default;
            const parser = this._typeParser(opt);
            const action = parser ? program.option(flagStr, desc, parser, def) : program.option(flagStr, desc, def);
            if (opt.alias) action.short(opt.alias);
        }

        let parseError = null;
        try { program.parse(['node', 'rarebert', ...argv]); } catch (err) { parseError = err; }
