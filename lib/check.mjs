...
import {
    buildBindingGraph as introspectBuildGraph,
    resolveClosure as introspectResolveClosure
} from './introspect.mjs';
...
const REGEX_KEYWORDS = new Set([
    ...
    'new', 'delete', 'void', 'throw', 'yield', 'await', 'if', 'for',
    'while', 'switch', 'with', 'try', 'catch', 'finally', 'function',
    'class', 'extends', 'implements', 'interface', 'package', 'private',
    'protected', 'public', 'static', 'let', 'const', 'var', 'export',
    'import', 'default', 'async', 'enum', 'as', 'from'
]);
            ...
            if (prevKind === 'start' || prevKind === 'op') {
                ...
                while (j < n) {
                    ...
                    if (c === '[') inClass = true;
                        ...
                        out.push(' ');
                        j++;
                        while (j < n && /[A-Za-z]/.test(content[j])) { out.push(' '); j++; }
                        break;
                    }
                    if (c === '\n') break;
                    out.push(' ');
...
function extractImportLocalNames(content) {
    ...
    while ((m = esmRe.exec(src)) !== null) {
        ...
        if (m[2]) {
            if (m[2].startsWith('*')) {
                ...
                for (const part of m[2].replace(/[{}]/g, '').split(',').map((s) => s.trim()).filter(Boolean)) {
    ...
    const cjsRe = /\b(?:const|let|var)\s+(\{[^}]*\}|[A-Za-z_$][\w$]*)\s*=\s*require\s*\(\s*['"][^'"]+['"]\s*\)/g;
    while ((m = cjsRe.exec(src)) !== null) {
        const lhs = m[1];
        if (lhs.startsWith('{')) {
            for (const part of lhs.replace(/[{}]/g, '').split(',').map((s) => s.trim()).filter(Boolean)) {
...
const CONTROL_KEYWORDS = new Set([
    ...
    'typeof', 'instanceof', 'in', 'of', 'new', 'await', 'yield', 'delete', 'void'
]);
...
const GLOBALS = new Set([
    ...
    'void', 'while', 'with', 'yield', 'async', 'await', 'of', 'as', 'from', 'enum',
    'implements', 'interface', 'package', 'private', 'protected', 'public',
    'arguments', 'eval', 'satisfies', 'keyof', 'infer', 'readonly', 'type',
    'namespace', 'declare', 'abstract', 'is',
    // JS global functions
    'parseInt', 'parseFloat', 'isNaN', 'isFinite', 'decodeURI', 'decodeURIComponent',
    'encodeURI', 'encodeURIComponent', 'escape', 'unescape',
    // JS builtins & Node globals
    'undefined', 'NaN', 'Infinity', 'globalThis', 'global', 'process', 'console',
    'Buffer', 'require', 'module', 'exports', '__dirname', '__filename',
    'setTimeout', 'setInterval', 'setImmediate', 'clearTimeout', 'clearInterval',
    'clearImmediate', 'queueMicrotask', 'performance', 'fetch', 'atob', 'btoa',
    'AbortController', 'AbortSignal', 'URL', 'URLSearchParams', 'TextEncoder',
    'TextDecoder', 'structuredClone', 'crypto', 'navigator', 'Blob', 'File',
    'FormData', 'Headers', 'Request', 'Response', 'ReadableStream',
    'WritableStream', 'TransformStream', 'MessageChannel', 'MessagePort',
    'Worker', 'BroadcastChannel', 'Event', 'EventTarget', 'CustomEvent',
    'Error', 'TypeError', 'RangeError', 'ReferenceError', 'SyntaxError',
    'EvalError', 'URIError', 'AggregateError', 'Promise', 'Array', 'String',
    'Number', 'Boolean', 'Symbol', 'BigInt', 'Object', 'Math', 'JSON', 'Date',
    'RegExp', 'Map', 'Set', 'WeakMap', 'WeakSet', 'WeakRef', 'Iterator',
    'Generator', 'AsyncGenerator', 'ArrayBuffer', 'SharedArrayBuffer', 'DataView',
    'Int8Array', 'Uint8Array', 'Uint8ClampedArray', 'Int16Array', 'Uint16Array',
    'Int32Array', 'Uint32Array', 'Float32Array', 'Float64Array', 'BigInt64Array',
    'BigUint64Array', 'Proxy', 'Reflect', 'Function', 'Intl', 'WebAssembly',
...
export {
    ...
    readBindings,
    resolveImportSource,
    introspectResolveClosure as resolveImportClosure,
    introspectBuildGraph as buildBindingGraph,
    runIntegrityChecks,
...
export default { runNodeCheck, buildBindingGraph: introspectBuildGraph, runIntegrityChecks, findUndefinedReferences, traceBinding };