...
import { listAllModules, cli, tui, promptModule, resolveModule, AbortError } from './module.mjs';
...
/**
 * Formatter for memo data structures.
 * This class handles all output formatting for memos while keeping the
 * core lib/memo.mjs functions pure data producers.
 */
class MemoFormatter {
    constructor(isJsonOutput = false) {
        this.isJsonOutput = isJsonOutput;
    }

    /**
     * Format a set of memo entries as JSON
     */
    formatMemosAsJson(entries) {
        return JSON.stringify(entries, null, 2);
    }

    /**
     * Print memos in flat format (one per line)
     */
    printFlat(modules, entries) {
        if (entries.length === 0) {
    ...
    /**
     * Print memos in DAG format with dependencies
     */
    printDag(modules, entries) {
        if (entries.length === 0) {
            console.log('No memos found.');
            return false;
        }

        // This is a simplified version - the full implementation will be added later
        for (const { module, memos } of entries) {
            console.log(`${module.path} has ${memos.length} memo(s):`);
    ...
    /**
     * Print success messages for added memos
     */
    printAdded(results) {
        for (const result of results) {
            if (result.success) {
                console.log(`${YELLOW_TICK} Memo added to ${result.path}`);
            } else {
                console.error(`Error adding memo to ${result.path}: ${result.error}`);
            }
        }
    }

    /**
     * Print commit confirmation message
     */
    printCommitConfirmation(snapshotLabel, hadMemos, isFresh) {
        if (!hadMemos) {
            console.log('No memos to commit.');
            return;
        }
        console.log(`Committed ${snapshotLabel} to git notes.`);
        if (isFresh) {
    ...
    /**
     * Print snapshot log entries
     */
    printLogEntries(entries, wanted = null) {
        if (entries.length === 0) {
     ...
     */
    printRestorationResults(results) {
        for (const result of results) {
            const since = result.lastModified
                ? ` (since ${new Date(result.lastModified).toISOString().slice(0, 10)})`
                : '';
            console.log(
                `memo: restored ${result.path} (${result.count} memo(s)${result.added ? `, +${result.added} new` : ''}${since})`
            );
        }
    }
     ...
     * Print drop results
     */
    printDropResults(count, path) {
        console.log(`Dropped ${count} memo(s) from ${path}`);
    }

    /**
     * Print forget results
     */
    printForgetResults(results) {
        for (const result of results) {
            if (result.success) {
                console.log(`${YELLOW_TICK} Forgot all memos for ${result.path}`);
            } else {
                console.error(`Error forgetting memos for ${result.path}: ${result.error}`);
            }
        }
    }
...
/**
 * Apply a drop operation to remove selected memos. Returns result data.
 */
export function applyDrop(resolved, selected) {
    ...
    return { path: resolved.rel, dropped: selected.length, sidecarDeleted };
}
...
/**
 * Print memos for a set of resolved modules. Returns structured data.
 * The script layer handles output formatting.
 */
export function cmdPrintSet(resolvedSet, withAncestors, asJson = false) {
    if (asJson) {
        const allMemos = memo.loadAllMemos();
        const setPaths = withAncestors ? null : new Set(resolvedSet.map((r) => r.rel));
        const result = {};
        for (const { module, memos, lastModified } of allMemos) {
            if (setPaths && !setPaths.has(module.path)) continue;
            result[module.path] = {
                memos: memos.map((c) => ({ content: c, lastModified })),
                lastModified
            };
        }
        return { action: 'print', format: 'json', data: result };
    }
    
    // Return raw data for script layer to format
    const allMemos = memo.loadAllMemos();
    const setPaths = withAncestors ? null : new Set(resolvedSet.map((r) => r.rel));
    const filtered = withAncestors ? allMemos : allMemos.filter((e) => setPaths.has(e.module.path));
    
    return { action: 'print', format: 'flat', entries: filtered };
}

/**
 * Print all memos. Returns structured data.
 * The script layer handles output formatting.
 */
export function cmdPrintAll(withAncestors = false, asJson = false) {
    if (asJson) {
        const allMemos = memo.loadAllMemos();
        const result = {};
        for (const { module, memos, lastModified } of allMemos) {
    ...
    // Return raw data for script layer to format
    const allMemos = memo.loadAllMemos();
    return { action: 'print', format: 'flat', entries: allMemos };
}

/**
 * Add memos to modules. Returns an array of result objects describing
 * what happened for each group processed. The script layer (scripts/memo.mjs)
...
export async function cmdAdd(groups, modules) {
    ...
    for (const g of groups) {
        ...
        if (!modArg) {
            results.push({ path: null, success: false, error: 'missing module path' });
            continue;
        }
        const content = rest.join(' ').trim();
        if (!content) {
            results.push({ path: modArg, success: false, error: `missing memo content for "${modArg}"` });
            continue;
        }
        const resolved = resolveModule(modArg, modules);
        if (!resolved) {
            results.push({ path: modArg, success: false, error: `module not found: ${modArg}` });
            continue;
        }
        memo.remember(resolved.rel, content);
        results.push({ path: resolved.rel, success: true, addedContent: content });
    }
    return { action: 'add', results };
}

/**
 * Commit memos to git notes. Returns an object describing the result.
 */
export async function cmdCommit(isYes, isFresh) {
    const label = 'memo snapshot';
    const hadMemos = memo.loadAllMemos().length > 0;
    if (!hadMemos) {
        return { action: 'commit', success: false, error: 'No memos to commit.' };
    }
    if (!isYes) {
        const ok = await tui.confirm('Commit memos to git notes?');
        if (!ok) {
            return { action: 'commit', success: false, aborted: true, message: 'Aborted; memos not committed.' };
        }
    }
    memo.snapshot(label);
    const freshMessage = isFresh ? ' (working sidecars cleared)' : '';
    memo.forgetAll();
    memo.clearBuffer();
    return { action: 'commit', success: true, message: `Memos committed${freshMessage}` };
}

/**
 * Log memo snapshots. Returns an array of snapshot entries.
 */
export function cmdLog(nonFlag) {
    const entries = memo.logEntries();
    if (entries.length === 0) {
        return { action: 'log', entries: [], message: 'memo: no snapshots in refs/notes/memos' };
    }
    ...
    for (const e of entries) {
        if (wanted) {
            const hits = e.modules.filter((m) => wanted.has(m));
            if (hits.length === 0) continue;
            result.push({ date: e.date, subject: e.subject, modules: hits });
        } else {
            result.push({ date: e.date, subject: e.subject });
        }
    }
    return { action: 'log', entries: result };
}

/**
 * Recall memos from a git notes snapshot. Returns info about the restoration.
 */
export function cmdRecall(ref, nonFlag) {
    if (!ref) {
        return { action: 'recall', success: false, error: 'missing ref argument' };
    }
    memo.restore(ref, nonFlag.length ? nonFlag : null);
    memo.clearBuffer();
    return { action: 'recall', success: true };
}

/**
 * Drop memos from a module. Returns structured result data.
 * For non-interactive (agent) mode, use indicesArg to specify memos to drop.
 * For interactive mode, the script layer can use multiSelectMemos() to prompt.
 */
export async function cmdDrop(moduleArg, indicesArg, modules) {
    if (!moduleArg) {
        return { action: 'drop', success: false, error: "A memo'd module must be specified for --drop." };
    }
    ...
    if (!allMemos.length) {
        return { action: 'drop', success: false, error: 'No memos found for this module.' };
    }

    // Non-interactive mode (agent mode) - must have indicesArg
    if (!cli.isInteractive()) {
        if (!indicesArg) {
            return {
                ...
                help: `Pass comma-separated indices (1 for first, -1 for last): --drop ${moduleArg} 1,3,-1\nOr to remove all memos for this module, use: --forget ${moduleArg}`
            };
        }
        const indices = parseIndices(indicesArg, allMemos.length);
        if (!indices) {
            return { action: 'drop', success: false, error: 'invalid indices' };
        }
        const selected = indices.map((i) => allMemos[i]);
        const dropped = await applyDrop(resolved, selected);
        return { action: 'drop', success: true, dropped, path: resolved.rel };
    }

    // Interactive mode - return data for script layer to handle TUI
    return {
        action: 'drop',
        success: 'needs_selection',
        modulePath: resolved.rel,
        allMemos,
        indicesArg
    };
}
...
/**
 * Forget memos from modules. Returns structured result data.
 */
export function cmdForget(moduleArgs, modules) {
    if (!moduleArgs || moduleArgs.length === 0) {
        return { action: 'forget', success: false, error: 'missing module argument' };
    }
    ...
    for (const moduleArg of moduleArgs) {
        ...
        if (!r) {
            return { action: 'forget', success: false, error: `module not found: ${moduleArg}` };
        }
        resolved.push(r);
    }
    ...
    for (const r of resolved) {
        if (!fs.existsSync(r.sidecar)) {
            results.push({ path: r.rel, status: 'not_found' });
            continue;
        }
        memo.forgetByPath(r.rel);
        results.push({ path: r.rel, status: 'forgotten' });
    }
    return { action: 'forget', results };
}
