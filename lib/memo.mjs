...
import { listAllModules, cli, tui, promptModule, resolveModule, AbortError, CLI, TUI } from './module.mjs';
import { git } from './git.mjs';
import { exit } from './core.mjs';
import { YELLOW_TICK, RED_BOLD, DIM, RESET, BOLD, BOLD_DIM } from './symbols.mjs';
    ...
    return new CLI(
        ...
        (args) => {
            if (args.bool('json')) {
                ...
                return exit(0, all.map((e) => ({ module: e.module.path, memos: e.memos.flatMap((m) => m.content) })));
            }
            if (withAncestors) {
        ...
        { name: 'memo-print-set', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}
...
export function cmdPrintAll(withAncestors = false) {
    return new CLI(
        ...
        { name: 'memo-print-all', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}

export function cmdAdd(groups, modules) {
    return new CLI(
        'memo-add',
        (args) => {
            ...
            for (const g of groups) {
                ...
                if (!modArg) return exit(1, json ? { action: 'add', ok: false, error: 'missing module path' } : 'memo --add: missing module path');
                const content = rest.join(' ').trim();
                if (!content) return exit(1, json ? { action: 'add', ok: false, error: `missing memo content for "${modArg}"` } : `memo --add: missing memo content for "${modArg}"`);
                const resolved = resolveModule(modArg, modules);
                if (!resolved) return exit(1, json ? { action: 'add', ok: false, error: `module not found: ${modArg}` } : `memo --add: module not found: ${modArg}`);
                memo.remember(resolved.rel, content);
                if (json) return exit(0, { action: 'add', ok: true, module: resolved.rel, memo: content });
                return exit(0, resolved.rel);
            }
            return exit(0);
        },
        { name: 'memo-add', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}

export function cmdCommit(isYes, isFresh) {
    return new CLI(
        'memo-commit',
        async (args) => {
            const json = args.bool('json');
            const hadMemos = memo.loadAllMemos().length > 0;
            if (!hadMemos) return exit(1, json ? { action: 'commit', ok: false, error: 'No memos to commit.' } : 'No memos to commit.');
            let confirmed = isYes;
            if (!isYes) {
                const ok = await tui.confirm('Commit memos to git notes?');
                confirmed = ok;
            }
            if (!confirmed) return exit(1, json ? { action: 'commit', ok: false, error: 'Aborted; memos not committed.' } : 'Aborted; memos not committed.');
            memo.snapshot('memo snapshot');
            if (isFresh) {
                memo.forgetAll();
                memo.clearBuffer();
                if (json) return exit(0, { action: 'commit', ok: true, fresh: true });
                return exit(0, 'Fresh slate (working sidecars cleared).');
            }
            memo.clearBuffer();
            if (json) return exit(0, { action: 'commit', ok: true });
            return exit(0);
        },
        { name: 'memo-commit', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}
...
export function cmdLog(nonFlag) {
    return new CLI(
        ...
        (args) => {
            ...
            if (entries.length === 0) return exit(1, json ? { action: 'log', ok: false, error: 'no snapshots' } : 'memo: no snapshots in refs/notes/memos');
            const wanted = nonFlag.length ? new Set(nonFlag) : null;
            const shownEntries = entries
                .filter((e) => !wanted || e.modules.some((m) => wanted.has(m)))
                .map((e) => {
                    if (wanted) {
                        const hits = e.modules.filter((m) => wanted.has(m));
                        return { date: e.date, subject: e.subject, modules: hits };
                    }
                    return { date: e.date, subject: e.subject };
                });
            if (json) return exit(0, { action: 'log', ok: true, entries: shownEntries });
            return exit(0, shownEntries);
        },
        { name: 'memo-log', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}
...
export function cmdRecall(ref, nonFlag) {
    return new CLI(
        ...
        (args) => {
            const json = args.bool('json');
            if (!ref) return exit(1, json ? { action: 'recall', ok: false, error: 'missing ref argument' } : 'memo --recall: missing ref argument');
            memo.restore(ref, nonFlag.length ? nonFlag : null);
            memo.clearBuffer();
            if (json) return exit(0, { action: 'recall', ok: true, ref });
            return exit(0);
        },
        { name: 'memo-recall', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}

export function cmdDrop(moduleArg, indicesArg, modules) {
    return new TUI(
        'memo-drop',
        async (args) => {
            const json = args.bool('json');
            if (!moduleArg) return exit(1, json ? { action: 'drop', ok: false, error: "A memo'd module must be specified for --drop." } : "A memo'd module must be specified for --drop.");
            ...
            if (!allMemos.length) return exit(1, json ? { action: 'drop', ok: false, error: 'No memos found for this module.' } : 'No memos found for this module.');

            if (process.stdin.isTTY !== true) {
                if (!indicesArg)
                    return exit(1, json ? { action: 'drop', ok: false, error: 'missing indices for non-interactive --drop' } : 'Error: missing indices argument for non-interactive --drop:\nNon-interactive mode cannot prompt for memo selection.\n  Pass comma-separated indices (1 for first, -1 for last): --drop ' + moduleArg + ' 1,3,-1\n  Or to remove all memos for this module, use: --forget ' + moduleArg);
                const indices = parseIndices(indicesArg, allMemos.length);
                if (!indices) return exit(1, json ? { action: 'drop', ok: false, error: 'Invalid indices provided.' } : 'Invalid indices provided.');
                applyDrop(resolved, indices.map((i) => allMemos[i]));
                if (json) return exit(0, { action: 'drop', ok: true, module: resolved.rel, dropped: indices.map((i) => allMemos[i]) });
                return exit(0);
            }
            ...
            if (!selected.length) return exit(1, json ? { action: 'drop', ok: false, error: 'No memos selected; nothing dropped.' } : 'No memos selected; nothing dropped.');

            applyDrop(resolved, selected);
            if (json) return exit(0, { action: 'drop', ok: true, module: resolved.rel, dropped: selected });
            return exit(0);
        },
        { name: 'memo-drop', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}
    ...
    return new CLI(
        ...
        (args) => {
            ...
            if (!moduleArgs || moduleArgs.length === 0)
                return exit(1, json ? { action: 'forget', ok: false, error: 'missing module argument' } : 'memo --forget: missing module argument');

            const resolved = [];
            for (const moduleArg of moduleArgs) {
                const r = resolveModule(moduleArg, modules);
                if (!r) return exit(1, json ? { action: 'forget', ok: false, error: 'module not found: ' + moduleArg } : 'memo --forget: module not found: ' + moduleArg);
                resolved.push(r);
            }

            if (json) {
                const forgotten = resolved.map((r) => ({
                    module: r.rel,
                    content: fs.existsSync(r.sidecar) ? memo.loadMemos(r.rel).flatMap((m) => m.content) : []
                }));
                for (const r of resolved) {
                    if (fs.existsSync(r.sidecar)) memo.forgetByPath(r.rel);
                }
                return exit(0, { action: 'forget', ok: true, forgotten });
            }

            const dropped = resolved.map((r) => ({
                module: r.rel,
                content: fs.existsSync(r.sidecar) ? memo.loadMemos(r.rel).flatMap((m) => m.content) : []
            }));
            return exit(0, () => {
                for (const r of resolved) {
                    if (fs.existsSync(r.sidecar)) {
                        ...
                        console.log(`No memos were found on ${r.rel}`);
                    }
                }
                console.dir({ dropped });
            });
        },
        { name: 'memo-forget', allowUnknownOption: true, options: [{ flag: '--json', description: 'JSON output' }] }
    );
}
