...
function moduleByPath(modulePath) {
    return modules().find((m) => m.path === modulePath) || null;
}

function memoFilePathFor(moduleRef) {
    if (isAbsolutePath(moduleRef)) {
        const mod = moduleByPath(moduleRef);
        return mod ? mod.path + SUFFIX : null;
    }

    const mod = modules().find((m) => m.name === moduleRef);
    return mod ? mod.path + SUFFIX : null;
}
