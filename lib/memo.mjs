...
class Memory {
    ...
    snapshot(label = 'memo snapshot') {
        ...
        // Structured meta-object format with clean subject line for git notes log
        const timestamp = Date.now();
        const modules = all.map((e) => ({
            path: e.module.path,
            name: e.module.name,
            memos: e.memos,
        ...
        // Subject line is clean for git notes log display (instead of verbose path list)
        const header = `${all.length} memo${all.length === 1 ? '' : 's'} cached`;
        const snapshotData = {
            version: 1,
            label,
            timestamp,
        ...
        const ok = git.notesAdd(payload, 'HEAD');
        if (ok) {
    ...
    showSnapshot(ref = 'HEAD') {
        ...
        // Parse using shared format detector
        return this._parseNoteContent(note);
    }

    _parseNoteContent(note) {
        // New meta-object format: "N memos cached\n\n{JSON}"
        const newlineIdx = note.indexOf('\n\n');
        
        if (newlineIdx >= 0) {
            const header = note.slice(0, newlineIdx);
            let payload = note.slice(newlineIdx + 2);

            // Check for new format: "N memos cached"
            if (/^\d+ memo(?:s)? cached$/.test(header.trim())) {
                try {
                    const data = JSON.parse(payload);
                    if (data && typeof data === 'object' && Array.isArray(data.modules)) {
                        return this._formatEntriesFromModules(data.modules);
                    }
                } catch {}
            }

            // Check for old format with header containing paths (comma-separated)
            // This was: "path1.mjs, path2.mjs\n\n[array]"
            if (!header.includes('memo') && !header.startsWith('{')) {
                // Legacy comma-separated paths approach - try parsing as JSON
                try {
                    const data = JSON.parse(payload);
                    if (Array.isArray(data)) return this._formatEntriesFromModules({ modules: data });
                } catch {}

                // Try direct array parse
                try {
                    const data = JSON.parse(note);
                    if (Array.isArray(data) && data[0]?.module?.path) {
                        return { legacy: false, entries: data.map(e => ({
                            module: e.module,
                            memos: Array.isArray(e.memos) ? e.memos : [],
                            lastModified: typeof e.lastModified === 'number' ? e.lastModified : 0
                        })) };
                    }

                    // Try as meta-object format (no header check needed)
                    if (data && typeof data === 'object' && Array.isArray(data.modules)) {
                        return this._formatEntriesFromModules(data);
                    }
                } catch {}
            } else {
            ...
            // Try parsing whatever's after the \n\n as JSON
            try {
                const data = JSON.parse(payload);
                if (data && typeof data === 'object' && Array.isArray(data.modules)) {
                    return this._formatEntriesFromModules(data);
                ...
                if (Array.isArray(data) && data[0]?.module?.path) {
                    return { legacy: false, entries: data };
                }
            } catch {}

            // Plain text lines fallback
            const lines = payload.split('\n').map(l => l.trim()).filter(Boolean);
            if (lines.length > 0 && !header.includes('memo')) {
                // Could be plain text memo notes format
                return { legacy: true, entries: lines };
            }
        }

        // No separator - try pure JSON parse
        try {
            const data = JSON.parse(note);
            if (data && typeof data === 'object' && Array.isArray(data.modules)) {
                return this._formatEntriesFromModules(data);
    ...
    _formatEntriesFromModules(modules) {
        if (!Array.isArray(modules)) return [];
        return modules.map((m) => ({
            module: { path: m.path, name: m.name },
    ...
    logEntries(limit = -1) {
        ...
        const entries = raw.map((e) => ({
            ...
            date: e.date,
            subject: e.subject,
            modules: this._parseModulesFromNote(e.hash, e.subject)
        }));
    ...
    _parseModulesFromNote(hash, subject) {
        if (!hash) return [];
        
        const note = git.notesShow(hash);
        if (!note) return [];

        // Try parsing from JSON body for new format
        const newlineIdx = note.indexOf('\n\n');
        let payload = null;
        let hasNewFormatHeader = false;

        if (newlineIdx >= 0) {
            const header = note.slice(0, newlineIdx);
            if (/^\d+ memo(?:s)? cached$/.test(header.trim())) {
                hasNewFormatHeader = true;
            }
            payload = note.slice(newlineIdx + 2);
        }
        ...
        if (payload) {
            try {
                const data = JSON.parse(payload);
                // New format with modules array
                if (data && typeof data === 'object' && Array.isArray(data.modules)) {
                    return data.modules.map(m => m.path).filter(Boolean);
                }
                // Legacy memo entry array format
                if (Array.isArray(data) && data[0]?.module?.path) {
                    return [...new Set(data.map(e => e.module?.path).filter(Boolean))];
                }
            } catch {}
        }

        // Fallback: try JSON without header check (pure JSON)
        ...
        // Old format fallback: subject contains comma-separated paths
        if (!hasNewFormatHeader) {
...
export async function cmdAdd(nonFlagArgs, modules) {
    if (nonFlagArgs.length === 0) {
        console.error('memo --add: missing module path');
        return;
    }
    if (nonFlagArgs.length === 1) {
        console.error(`memo --add: missing memo content for "${nonFlagArgs[0]}"`);
        return;
    }
    // The module positional is the one that resolves to a known module;
    // the rest form the memo text. Either may come first.
    let modArg = null;
    let modIdx = -1;
    for (let i = 0; i < nonFlagArgs.length; i++) {
        if (resolveModule(nonFlagArgs[i], modules)) {
            modArg = nonFlagArgs[i];
            modIdx = i;
            break;
        }
    }
    if (modArg === null) {
        // Fall back to first positional; resolveModule will report not found.
        modArg = nonFlagArgs[0];
        modIdx = 0;
    }
    const memoParts = nonFlagArgs.filter((_, i) => i !== modIdx);
    const content = memoParts.join(' ').trim();
    if (!content) {
...
/**
 * Parse a raw git note (memo snapshot) into a flat list of
 * `{ module, memos }` pairs suitable for display (e.g. trail).
 * Handles the new "N memos cached\n\n{JSON}" header format, the
 * legacy comma-separated-paths header, legacy memo-entry arrays,
 * and plain-text fallback. Returns `[]` for missing/empty/unparseable
 * notes — callers can treat falsy as "no memos".
 */
export function parseNoteMemos(note) {
    if (!note) return [];
    const parsed = memo._parseNoteContent(note);
    if (!parsed) return [];
    if (parsed.legacy) return [];
    const labelOf = (m) => m?.name || m?.path || 'unknown';
    return parsed.entries.map((e) => ({
        module: labelOf(e.module),
        memos: Array.isArray(e.memos) ? e.memos : []
    }));
}
