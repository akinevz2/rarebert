...
import { listAllModules, cli, promptModule, resolveModule, AbortError, Interface } from './module.mjs';
...
export async function cmdCommit(isYes, isFresh) {
    ...
    if (!isYes) {
        // cmdCommit can run non-interactively (memo --commit without --yes),
        // so guard before constructing the Interface.
        if (!cli.isInteractive()) {
            console.log('Aborted; memos not committed (non-interactive).');
            return;
        }
