...
export function cmdForget(moduleArgs, modules) {
    if (!moduleArgs || moduleArgs.length === 0) return exit('memo --forget: missing module argument');
    ...
    const dropped = resolved.map((r) => ({
        module: r.rel,
        content: fs.existsSync(r.sidecar)
            ? memo.loadMemos(r.rel).flatMap((m) => m.content)
            : []
    }));
    return exit(0, () => {
