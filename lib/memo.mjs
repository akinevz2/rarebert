...
class Memory {
    ...
    walkAll() {
        const groups = [];
        // collect memos for all modules
        const memos = this.modules().map((m) => {
            return this.loadMemosWithTimestamps(m.path);
        });
        // sort oldest to newest
        const oldestToNewest = (key) => (a, b) => {
            if (!(key in a && key in b))
                throw new Error(`can't sort {${a}<>${b}}, missing comparable field ${key}`);
            return a[key] - b[key];
        };
        const memosByAge = [...memos].sort(oldestToNewest('lastModified'));
        // starting from the oldest memo to newest
        // resolve full tree of imports for owning module
        // load each module's memos, if not yet loaded
        const memosWithImports = memosByAge.flatMap((m) => {
            const imports = parseImports(m.content.abs);
        });
        for (const m of this.modules()) {
            ...
            groups.push({ module: m, memos: own.flatMap((memo) => memo.content), libs });
        }
        groups.sort((a, b) => {
