...
class Memory {
    ...
    _relatedMemos(mod) {
        ...
        const stamped = related.map((rel) => {
            ...
            try {
                const data = JSON.parse(fs.readFileSync(file, 'utf-8'));
                return { rel, lastModified: typeof data.lastModified === 'number' ? data.lastModified : 0 };
            } catch {
     ...
     *
     * For each owner module that has a memo sidecar, produce:
     *   { owner, path, memos: string[], related: RelatedEntry[], cycles: number }
     *
     * `related` lists every memoised import (oldest memo first). Each
     * RelatedEntry is { path, cycle: boolean } — cycle is true when the
     * import refers back to a module already visited on the current
     * walk (a back-edge).
     *
     * Deduplication: each module path appears at most once as an owner
     * in the returned list. Back-edges are not followed but are flagged.
     */
    walkAll() {
        const oldestToNewestByKey = (key) => (a, b) => {
            if (!(key in a && key in b))
        ...
        for (const m of allModules) {
            if (visited.has(m.path)) continue;
            const own = this.loadMemos(m.path);
            if (own.length === 0) continue;

            const ownerMemo = own[0];
            visited.add(m.path);

            const related = [];
            let cycles = 0;
            for (const relPath of ownerMemo.related) {
                if (visited.has(relPath) || relPath === m.path) {
                    cycles++;
        ...
        // sort by owner module lastModified (oldest first)
        groups.sort((a, b) => {
            const ta = this._moduleLastModified(a.path);
            const tb = this._moduleLastModified(b.path);
            return ta - tb;
        });
        return groups;
    }
