...
export function recallImports(callerPath) {
...
function parseImports(filePath) {
    ...
    const names = new Set();
    const re = /(?:import\s+(?:\*\s+as\s+\w+|\w+(?:\s*,\s*\{[^}]*\})?|\{[^}]*\})\s+from|import\s+)(['"`])([^'"`]+)\1/g;
    let m;
...
export default { memos, remember, forget, loadMemos, recallImports };