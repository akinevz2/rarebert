...
export function recallImports(callerRef) {
    ...
    for (const libName of imports) {
        for (const content of loadMemos(libName)) {
            if (memoCascadingBuffer.some(m => m.name === libName && m.content === content)) continue;
...
export default { memoCascadingBuffer, remember, forget, loadMemos, recallImports, flush, installFlushHandlers, loadForRun };