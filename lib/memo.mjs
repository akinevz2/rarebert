...
const remember = (name, content) => memo.remember(name, content);
const forget = (name) => memo.forget(name);
const forgetAllMemos = () => memo.forgetAll();
const forgetMemos = (name, recursive) => memo.forgetMemos(name, recursive);
const loadMemos = (name) => memo.loadMemos(name);
const loadAllMemos = () => memo.loadAllMemos();
const clearBuffer = () => memo.clearBuffer();
const recallImports = (callerRef) => memo.recallImports(callerRef);
const flush = () => memo.flush();
const installFlushHandlers = () => memo.installFlushHandlers();
const loadForRun = (scriptPath, scriptName) => memo.loadForRun(scriptPath, scriptName);
