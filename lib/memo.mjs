...
// ---------------------------------------------------------------------------
// cmd*() — simple Module declarations. Each reads its inputs from `args`
// (the ModuleArguments), performs the work, and returns:
//   exit(0)              — success, no data
//   exit('error string') — failure
//   exit(0, data)        — success with produced data (propagated to caller)
// No --json check here — scripts/memo.mjs decides presentation.
// ---------------------------------------------------------------------------

export const cmdPrintSet = new CLI(
    'memo-print-set',
    (args) => {
        const resolvedSet = args.resolvedSet || [];
    ...
    { name: 'memo-print-set', allowUnknownOption: true }
);

export const cmdPrintAll = new CLI(
    'memo-print-all',
    (args) => {
        const all = memo.loadAllMemos();
        return exit(0, all.map((e) => ({ module: e.module.path, memos: e.memos.flatMap((m) => m.content) })));
    },
    { name: 'memo-print-all', allowUnknownOption: true }
);

export const cmdAdd = new CLI(
    'memo-add',
    (args) => {
        ...
        for (const g of groups) {
            ...
            if (!modArg) return exit('memo --add: missing module path');
            const content = rest.join(' ').trim();
            if (!content) return exit(`memo --add: missing memo content for "${modArg}"`);
            const resolved = resolveModule(modArg, modules);
            if (!resolved) return exit(`memo --add: module not found: ${modArg}`);
            memo.remember(resolved.rel, content);
            return exit(0, { module: resolved.rel, memo: content });
        }
        return exit(0);
    },
    { name: 'memo-add', allowUnknownOption: true }
);

export const cmdCommit = new CLI(
    'memo-commit',
    async (args) => {
        const isYes = args.bool('yes');
        const isFresh = args.bool('fresh');
        const hadMemos = memo.loadAllMemos().length > 0;
        if (!hadMemos) return exit('No memos to commit.');
        let confirmed = isYes;
        if (!isYes) {
            const ok = await tui.confirm('Commit memos to git notes?');
            confirmed = ok;
        }
        if (!confirmed) return exit('Aborted; memos not committed.');
        memo.snapshot('memo snapshot');
        if (isFresh) {
            memo.forgetAll();
            memo.clearBuffer();
            return exit(0, { fresh: true });
        }
        memo.clearBuffer();
        return exit(0);
    },
    { name: 'memo-commit', allowUnknownOption: true }
);

export const cmdLog = new CLI(
    'memo-log',
    (args) => {
        const nonFlag = args.nonFlag();
        const entries = memo.logEntries();
        if (entries.length === 0) return exit('memo: no snapshots in refs/notes/memos');
    ...
    { name: 'memo-log', allowUnknownOption: true }
);

export const cmdRecall = new CLI(
    'memo-recall',
    (args) => {
        const nonFlag = args.nonFlag();
        const ref = nonFlag[0];
        if (!ref) return exit('memo --recall: missing ref argument');
        memo.restore(ref, nonFlag.length > 1 ? nonFlag.slice(1) : null);
        memo.clearBuffer();
        return exit(0, { ref });
    },
    { name: 'memo-recall', allowUnknownOption: true }
);

export const cmdDrop = new TUI(
    'memo-drop',
    async (args) => {
        const nonFlag = args.nonFlag();
        const moduleArg = nonFlag[0];
        const indicesArg = nonFlag[1];
        const modules = listAllModules();
        if (!moduleArg) return exit("A memo'd module must be specified for --drop.");
        ...
        if (!allMemos.length) return exit('No memos found for this module.');

        if (process.stdin.isTTY !== true) {
            if (!indicesArg)
                return exit(
                    'Error: missing indices argument for non-interactive --drop:\nNon-interactive mode cannot prompt for memo selection.\n  Pass comma-separated indices (1 for first, -1 for last): --drop ' +
                        moduleArg + ' 1,3,-1\n  Or to remove all memos for this module, use: --forget ' + moduleArg
                );
            const indices = parseIndices(indicesArg, allMemos.length);
            if (!indices) return exit('Invalid indices provided.');
            const dropped = indices.map((i) => allMemos[i]);
            applyDrop(resolved, dropped);
            return exit(0, { module: resolved.rel, dropped });
        }
        ...
        if (!selected.length) return exit('No memos selected; nothing dropped.');

        applyDrop(resolved, selected);
        return exit(0, { module: resolved.rel, dropped: selected });
    },
    { name: 'memo-drop', allowUnknownOption: true }
);

export const cmdForget = new CLI(
    'memo-forget',
    (args) => {
        const moduleArgs = args.nonFlag();
        const modules = listAllModules();
        if (!moduleArgs || moduleArgs.length === 0)
            return exit('memo --forget: missing module argument');

        const resolved = [];
        for (const moduleArg of moduleArgs) {
            const r = resolveModule(moduleArg, modules);
            if (!r) return exit('memo --forget: module not found: ' + moduleArg);
            resolved.push(r);
        }
        ...
        return exit(0, { forgotten });
    },
    { name: 'memo-forget', allowUnknownOption: true }
);
