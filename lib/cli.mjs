...
const onAbort = (cb) => cli.onAbort(cb);
const runAbortCallbacks = () => cli.runAbortCallbacks();
const installSignalHandlers = () => cli.installSignalHandlers();
const die = (msg, code) => cli.die(msg, code);
const abort = (msg) => cli.abort(msg);
const ok = (msg) => cli.ok(msg);
const fail = (msg) => cli.fail(msg);
const isInteractive = () => cli.isInteractive();
const nonInteractive = (msg, code) => cli.nonInteractive(msg, code);
const confirm = (msg, initial) => cli.confirm(msg, initial);
const input = (msg, opts) => cli.input(msg, opts);
const select = (msg, choices, opts) => cli.select(msg, choices, opts);
const parseArgs = (argv, spec) => cli.parseArgs(argv, spec);
const wantsHelp = (args) => cli.wantsHelp(args);
const printHelp = (meta) => cli.printHelp(meta);
const run = (meta, main) => cli.run(meta, main);
