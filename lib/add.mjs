...
function projectChoices() {
    return current.discover().map((p) => ({ name: p.key, message: p.label }));
}
...
async function promptModuleName(lang) {
    ...
    const namePrompt = new Enquirer.Input({
        ...
        validate: (val) => {
            ...
            try {
                current.normalizeModuleName(val, [ext]);
                return true;
            } catch (e) {
                return e.message;
            }
...
async function scaffoldSrcModule(lang, moduleName) {
    ...
    const modulePath = path.join(current.srcDir, `${moduleName}${ext}`);
    if (fs.existsSync(modulePath)) cli.fail(`${moduleName}${ext} already exists in src/`);
    fs.mkdirSync(current.srcDir, { recursive: true });
    const content = (
