...
// REQUEST: pickLanguage, promptProjectName, scaffoldSrcModule are used by scripts/add.mjs.
...
async function pickLanguage(defaultLang = 'mjs') {
    if (!cli.isInteractive()) return cli.nonInteractive('cannot pick a language.');
    const langs = languages.list();
    if (langs.length === 0) return await installNewLanguage();
    const initial = Math.max(0, langs.indexOf(defaultLang));
    const choice = await tui.select('Select a language for the new module:', languageChoices(), {
