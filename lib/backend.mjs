...
const OPENCODE_CONFIG = path.join(rarebert.root, 'opencode.jsonc');
...
class Backend {
    ...
    async runOnboard({ force = false } = {}) {
        if (!force && this.isConfigured()) {
            console.log('onboard: opencode.jsonc already configured; re-run with --force to reconfigure.');
            return true;
        }

        if (!cli.isInteractive()) {
            console.error(
                'onboard: opencode.jsonc is not configured and stdin is not a TTY; ' +
        ...
        if (force && fs.existsSync(this.configPath)) {
            ...
            console.log('Reconfiguring existing opencode.jsonc.\n');
        } else {
            console.log('\n=== rarebert onboarding ===');
            console.log("No usable opencode.jsonc found. Let's configure a model.\n");
        }
        ...
        console.log(`\n✓ Wrote ${path.relative(rarebert.root, this.configPath)}`);
     ...
     * etc.) or a graphical one (code, subl, ...). Stored in rarebert's own
     * prefs file (not opencode.jsonc) so `make open` knows whether
    ...
    /**
     * Read rarebert's own preferences from the prefs file in the data
     * directory. Returns an empty object if the file is missing or
     * malformed. This is separate from opencode.jsonc so rarebert
     * never writes its own keys into the user's opencode config.
     */
    readPrefs() {
    ...
    writePrefs(prefs) {
        ...
        fs.writeFileSync(PREFS_FILE, JSON.stringify(prefs, null, 4) + '\n');
    }

    setEditorType(editorType) {
        const prefs = this.readPrefs();
        prefs.editorType = editorType;
        this.writePrefs(prefs);
    }

    /**
     * Read the persisted `editorType` preference ('graphical' or
     * 'terminal'). Returns null if unset or the prefs file is missing.
     */
    getEditorType() {
        const prefs = this.readPrefs();
        const t = prefs.editorType;
        return t === 'terminal'
            ? 'terminal'
            : t === 'graphical'
              ? 'graphical'
    ...
    // -----------------------------------------------------------------------
    // Project folder scanning — used by `make add` to discover
    // module-containing folders for registration.
    // -----------------------------------------------------------------------
    ...
    /**
     * Ensure the opencode config is complete. Called by the dispatcher
     * before running any module. Project registration is handled
     * separately by `make add`.
     */
    async ensureAll() {
