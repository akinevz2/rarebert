...
class Backend {
    ...
    async runOnboard({ force = false } = {}) {
        ...
        if (discovered.length > 0) {
            ...
            console.log('  Start Ollama and re-run `make backend` to complete configuration.');
            return false;
        }
    ...
    async ensureConfig({ force = false } = {}) {
        ...
        if (this.hasPendingModels()) {
            ...
            console.log(`backend: retrying ${baseURL}/models ...`);
            const discovered = this.fetchModels(baseURL);
            if (discovered.length === 0) {
                console.error(
                    `backend: Ollama at ${baseURL} is still unreachable.\n` +
                        'Start Ollama and re-run `make backend` to complete configuration.'
                );
                return false;
            }
            ...
            if (!cli.isInteractive()) {
                console.error(
                    'backend: models found but stdin is not a TTY; run `make backend` interactively.'
                );
                return false;
            }
