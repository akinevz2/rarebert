...
class Backend {
    ...
    async promptModelId(discovered) {
        if (discovered.length > 0) {
            ...
            const picked = await cli.select('Pick a model:', choices, { initial: 0 });
            if (picked === '__custom__') {
                return await cli.input('Model id (e.g. glm-5.2:cloud, llama3:latest):', {
    ...
    async runOnboard({ force = false } = {}) {
        ...
        const useDefault = await cli.confirm(
        ...
        const modelName = await cli.input('Display name for this model (optional):', {
            initial: modelId
        });
