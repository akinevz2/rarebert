...
import { cli, tui, AbortError } from './module.mjs';
...
class Backend {
    ...
    async promptBaseURL(initial = DEFAULT_BASE_URL) {
        return await tui.input('Ollama endpoint URL (OpenAI-compatible /v1):', {
            initial,
    ...
    async promptModelIds(discovered) {
        ...
        const id = await tui.input('Model id (e.g. glm-5.2:cloud, llama3:latest):', {
            validate: (v) => (v.trim() ? true : 'Model id is required')
        });
        return [id];
    }
    ...
    async promptDefaultModel(modelIds) {
        ...
        return await tui.select('Which model should be the default?', choices, { initial: 0 });
    }
    ...
    async promptModelDisplayNames(modelIds) {
        ...
        const giveNames = await tui.confirm('Give custom names to any of the models?', false);
        if (!giveNames) return models;

        for (const m of models) {
            const name = await tui.input(`Display name for ${m.id} (optional):`, {
                initial: m.id
            });
    ...
    async runOnboard({ force = false } = {}) {
        ...
        const baseURLInput = await tui.input(`Ollama endpoint URL (empty = ${initialURL}):`, {
            initial: '',
            validate: (v) =>
     ...
     * current project.
     */
    async runOnboardNonInteractive({ baseURL, model, provider = DEFAULT_PROVIDER, editorType = null, force = false } = {}) {
        if (!force && this.isConfigured()) return true;
        ...
        if (discovered.length > 0) {
            ...
            if (discovered.includes(model)) {
                ...
                console.log(`  specified model "${model}" not in discovered list; writing config anyway.`);
