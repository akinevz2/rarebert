#!/usr/bin/env node
// formatReport/printUsage merged from lib/refactor.mjs (this script was its only consumer).
...
const subcommands = {
    ...
    cleanup:
        'Post-commit cleanup: drop stale pre-snapshot memos and confirm new summaries on affected modules'
};
        ...
        switch (effectiveSub) {
            ...
            case 'damage': {
                ...
                if (!noMemos) {
                    console.error('\n┌─ memo generation ────────────────────────────');
                    console.error(
                        `│ Generating damage summaries via opencode${model ? ' (' + model + ')' : ''}...`
                    );
            ...
            case 'cleanup': {
                ...
                if (summary.length === 0) {
                    ...
                    console.log(
                        `  Done: ${totalDropped} stale memo(s) dropped across ${summary.length} module(s).`
                    );
                }
                return exit(0);
            }
