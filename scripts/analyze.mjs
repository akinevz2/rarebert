...
import { listAllModules, resolveModule, cli, CLI } from '../lib/module.mjs';
...
\`\`\``;
    ...
    const raw = ide.runOpencodeHeadless(instruction, model);
    if (!raw) return [];
...
${code}
\`\`\``;
    return ide.runOpencodeHeadless(instruction, model);
}
...
/**
 * Print code intelligence for a module as a neat list — imports, main()
 * segmentation (line spans only, no opencode), and public members. No
 * network calls, no prompts. This is the default `analyze` behaviour.
 */
async function printIntelligence(relPath, content, ext) {
    ...
    const members = await languages.extractPublicMembers(content, ext);
    ...
    if (mainFunc) {
        console.log('\n--- main() ---');
        console.log(
            `  ${relPath}:${mainFunc.startLine}-${mainFunc.endLine} (+${mainFunc.bodyLines.length} lines)`
        );
    } else {
        console.log('\n--- main() ---');
    ...
    if (members.length === 0) {
        ...
        for (const m of members) {
            const span = `${relPath}:${m.startLine}-${m.endLine} (+${m.endLine - m.startLine + 1})`;
            console.log(`  ${span}  [${m.kind} ${m.name}]`);
        }
    }
    console.log();
...
async function main(opts = {}, positional = []) {
    ...
    let resolved;
    if (moduleArg) {
        resolved = resolveModule(moduleArg, listAllModules());
    } else {
        ...
        const selection = await cli.select('Select a module to analyze:', choices, {
            nonInteractiveBehavior: 'fail'
        });
        resolved = resolveModule(selection, modules);
    }
    if (!resolved) throw new Error(`Module not found: ${moduleArg}`);
...
export default new CLI('analyze.mjs', main, meta).supportsDirectRunning(import.meta.url);
