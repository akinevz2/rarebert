...
PY_TARGETS := add-java add-notification add-repo-error agent check-hosts data-loader dev devlib get-knowledge-file get-training-set get-usable-hosts hyper-analysis hyper-tag notify query reminders rip-internet-movie runtime scan visualise-data wow-discussion wow-list-sessions
...
runtime:
	@$(PYTHON) runtime.py $(ARGS) $(ARG) $(if $(HOST),"HOST=$(HOST)",) $(if $(HOSTS),"HOSTS=$(HOSTS)",) $(if $(PORT),"PORT=$(PORT)",) $(if $(WHOM),"WHOM=$(WHOM)",) $(if $(WHERE),"WHERE=$(WHERE)",) $(if $(ASK),"ASK=$(ASK)",) $(if $(TOPIC),"TOPIC=$(TOPIC)",) $(if $(SESSION),"SESSION=$(SESSION)",) $(if $(RESET),"RESET=$(RESET)",) $(if $(SEARCH),"SEARCH=$(SEARCH)",) $(if $(RECALL),"RECALL=$(RECALL)",) $(if $(LIST),"LIST=$(LIST)",) $(if $(DB),"DB=$(DB)",) $(if $(FILE),"FILE=$(FILE)",) $(if $(URL),"URL=$(URL)",) $(if $(LIMIT),"LIMIT=$(LIMIT)",) $(if $(THRESHOLD),"THRESHOLD=$(THRESHOLD)",) $(if $(BINDING),"BINDING=$(BINDING)",) $(if $(TOKENS),"TOKENS=$(TOKENS)",)
